...
class PyroelectricAppGUI:
    ...
    FONT_LABEL_BOLD = ('Segoe UI', FONT_SIZE_BASE, 'bold')
    FONT_TITLE = ('Segoe UI', FONT_SIZE_BASE + 4, 'bold'); FONT_CONSOLE = ('Consolas', 10)
    ...
    def setup_styles(self):
        ...
        style.configure('TPanedWindow', background=self.CLR_BG_DARK, sashwidth=8)
        style.configure('TLabel', background=self.CLR_BG_DARK, foreground=self.CLR_FG_LIGHT, font=self.FONT_BASE)

        # --- Buttons ---
        style.configure('TButton', font=self.FONT_LABEL_BOLD, padding=(10, 8), borderwidth=0)
        style.map('TButton',
            foreground=[('!active', self.CLR_BG_DARK), ('active', self.CLR_BG_DARK)],
            background=[('!active', self.CLR_ACCENT_BLUE), ('pressed', self.CLR_FG_LIGHT), ('hover', self.CLR_FG_LIGHT)]
        )
        ...
        # --- Entry and Combobox ---
        style.configure('TEntry', fieldbackground=self.CLR_FG_LIGHT, foreground=self.CLR_BG_DARK, borderwidth=1, insertcolor=self.CLR_BG_DARK)
        style.configure('TCombobox', fieldbackground=self.CLR_FG_LIGHT, foreground=self.CLR_BG_DARK, borderwidth=1, arrowcolor=self.CLR_BG_DARK)

        # --- LabelFrame ---
        style.configure('TLabelframe', background=self.CLR_BG_DARK, borderwidth=1, relief='solid')
        style.configure('TLabelframe.Label', background=self.CLR_BG_DARK, foreground=self.CLR_ACCENT_BLUE, font=self.FONT_LABEL_BOLD)
    ...
    def create_input_frame(self, parent):
        ...
        self.entries = {}
        # Side-by-side layout for temperature inputs
        ttk.Label(frame, text="Start Temp (K):", font=self.FONT_LABEL_BOLD).grid(row=1, column=0, padx=(15,5), pady=(10,0), sticky='w')
        self.entries['Start Temp'] = ttk.Entry(frame, font=self.FONT_BASE, width=15); self.entries['Start Temp'].grid(row=2, column=0, padx=(15,5), pady=(0,10), sticky='ew')

        ttk.Label(frame, text="End Temp (K):", font=self.FONT_LABEL_BOLD).grid(row=1, column=1, padx=(5,15), pady=(10,0), sticky='w')
        self.entries['End Temp'] = ttk.Entry(frame, font=self.FONT_BASE, width=15); self.entries['End Temp'].grid(row=2, column=1, padx=(5,15), pady=(0,10), sticky='ew')

        # Side-by-side layout for rate and safety
        ttk.Label(frame, text="Ramp Rate (K/min):", font=self.FONT_LABEL_BOLD).grid(row=3, column=0, padx=(15,5), pady=(5,0), sticky='w')
        self.entries['Ramp Rate'] = ttk.Entry(frame, font=self.FONT_BASE, width=15); self.entries['Ramp Rate'].grid(row=4, column=0, padx=(15,5), pady=(0,15), sticky='ew')

        ttk.Label(frame, text="Safety Cutoff (K):", font=self.FONT_LABEL_BOLD).grid(row=3, column=1, padx=(5,15), pady=(5,0), sticky='w')
        self.entries['Safety Cutoff'] = ttk.Entry(frame, font=self.FONT_BASE, width=15); self.entries['Safety Cutoff'].grid(row=4, column=1, padx=(5,15), pady=(0,15), sticky='ew')

        # Sample Name and VISA inputs
        ttk.Label(frame, text="Sample Name:", font=self.FONT_LABEL_BOLD).grid(row=0, column=0, columnspan=2, padx=15, pady=(10,0), sticky='w')
        self.entries['Sample Name'] = ttk.Entry(frame, font=self.FONT_BASE); self.entries['Sample Name'].grid(row=0, column=1, padx=(5,15), pady=(10,10), sticky='ew')

        ttk.Label(frame, text="Lakeshore 350 VISA:", font=self.FONT_LABEL_BOLD).grid(row=5, column=0, columnspan=2, padx=15, pady=(5,0), sticky='w')
        self.lakeshore_combobox = ttk.Combobox(frame, font=self.FONT_BASE, state='readonly'); self.lakeshore_combobox.grid(row=6, column=0, columnspan=2, padx=15, pady=(0,5), sticky='ew')

        ttk.Label(frame, text="Keithley 6517B VISA:", font=self.FONT_LABEL_BOLD).grid(row=7, column=0, columnspan=2, padx=15, pady=(5,0), sticky='w')
        self.keithley_combobox = ttk.Combobox(frame, font=self.FONT_BASE, state='readonly'); self.keithley_combobox.grid(row=8, column=0, columnspan=2, padx=15, pady=(0,15), sticky='ew')

        # Buttons in a 2x2 grid
        button_frame = ttk.Frame(frame); button_frame.grid(row=9, column=0, columnspan=2, padx=10, pady=10, sticky='ew')
        for i in range(2): button_frame.grid_columnconfigure(i, weight=1)
        self.scan_button = ttk.Button(button_frame, text="Scan Instruments", command=self._scan_for_visa_instruments); self.scan_button.grid(row=0, column=0, padx=5, pady=5, sticky='ew')
        self.file_location_button = ttk.Button(button_frame, text="Browse Save Location", command=self._browse_file_location); self.file_location_button.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        self.start_button = ttk.Button(button_frame, text="START", command=self.start_measurement, style='Start.TButton'); self.start_button.grid(row=1, column=0, padx=5, pady=5, sticky='ew')
        self.stop_button = ttk.Button(button_frame, text="STOP", command=self.stop_measurement, style='Stop.TButton', state='disabled'); self.stop_button.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
    ...
    def create_graph_frame(self, parent):
        ...
        self.ax_main.set_title("Current vs. Temperature", fontsize=14, weight='bold'); self.ax_main.set_xlabel("Temperature (K)"); self.ax_main.set_ylabel("Current (A)")

        self.line_sub1, = self.ax_sub1.plot([], [], color='#0077b6', marker='.', markersize=4, linestyle='-', linewidth=1)
        self.ax_sub1.set_title("Temperature vs. Time", fontsize=11); self.ax_sub1.set_xlabel("Time (s)"); self.ax_sub1.set_ylabel("Temperature (K)")

        self.line_sub2, = self.ax_sub2.plot([], [], color='#06d6a0', marker='.', markersize=4, linestyle='-', linewidth=1)
        self.ax_sub2.set_title("Current vs. Time", fontsize=11); self.ax_sub2.set_xlabel("Time (s)"); self.ax_sub2.set_ylabel("Current (A)")

        for ax in self.axes: ax.set_facecolor('#f7f7f7'); ax.ticklabel_format(axis='y', style='sci', scilimits=(-2, 3), useMathText=True)
    ...
    def start_measurement(self):
        try:
            ...
            self.ax_main.set_title(f"I vs T | Sample: {params['sample_name']}", fontsize=14, weight='bold'); self.canvas.draw()
        ...
        try:
            ...
            if self.experiment_state == 'stabilizing':
                ...
                if abs(current_temp - params['start_temp']) < 0.1:
                    self.log(f"Stabilized at {params['start_temp']} K. Starting ramp."); self.experiment_state = 'ramping'; self.backend.start_ramp(); self.start_time = time.time()
            ...
            self.log(f"RUNTIME ERROR: {traceback.format_exc()}"); self.stop_measurement(reason="runtime error"); messagebox.showerror("Runtime Error", "An error occurred. Check console for details."); return
