"""
===============================================================================
 PROGRAM:      AC R-T (T Sensing) - Keithley 6221 + SR830 + Lakeshore 350
 PURPOSE:      AC resistance against temperature, logged passively while
               ...
               TEMPERATURE
               -----------
               This module never writes to the Lakeshore 350. Whatever is
               driving the temperature -- a ramp started from the Lakeshore's
               own front panel, a PICA temperature control module, or a
               cryostat left to warm on its own -- keeps driving it, and the
               run simply records KRDG? against the AC resistance. The run
               ends when the temperature leaves the stop window, or when it is
               stopped by hand.
...
PROGRAM_VERSION = "1.0"
MODULE_NAME = "RT_AC_K6221_SR830_L350_T_Sensing_GUI.py"
PROGRAM_TITLE = "AC R-T (T Sensing) - Keithley 6221 + SR830 + Lakeshore 350"
...
INFO_TEXT = (
    ...
    "  (voltage), Lakeshore 350 (read only)\n"
    ...
    "\n"
    "Nothing here writes to the Lakeshore. Drive the temperature\n"
...
class Lakeshore350Monitor:
    """Read-only thermometry from a Lakeshore 350.

    Nothing in this class writes to the instrument. Whatever is driving the
    temperature -- a Lakeshore ramp started from its own front panel, a
    cryostat, a dewar warming up on its own -- keeps driving it, and this
    module only watches. KRDG? returns Kelvin regardless of what the front
    panel is displaying, so there is no units trap here as there is on the
    Cryocon.
    """
    ...
    def __init__(self, visa_address, channel="A"):
        ...
        if not is_lakeshore_idn(self.idn):
            try:
                ...
                self.instrument = None
            raise ConnectionError(
                "%s is not a Lakeshore: it identifies itself as '%s'. Scan "
                "the bus and use the Lakeshore's actual address."
...
class ACResistanceTSensingGUI:
    ...
    def _connect_worker(self, addresses):
        ...
        try:
            with self.io_lock:
                thermometer = Lakeshore350Monitor(
