"""
Module: RT_AC_K6221_SR830_L340_T_Sensing_GUI.py
Port of RT_AC_K6221_SR830_L350_T_Sensing_GUI.py to the Model 340 command set.

What is different from the Model 350 version, and why
-----------------------------------------------------
  * *IDN? must contain MODEL340 or Connect is refused.  The 350 version
    accepted any LSCI reply; the lab's 340, 350, Cryocon 34 and 6221
    all share factory address 12, so a "Lakeshore-looking" address
    proves nothing, and RANGE 0 must never reach the wrong box.
  * *CLS only at run start.  Never *RST: on a 340 "*RST sets
    controller parameters to power-up settings" (340 manual, printed
    9-24); it can change the control loop, setpoint and ramp that
    something else is using to drive the temperature.
  * Heater-off at start is a checkbox, ON by default.  When ticked the
    run sends "RANGE 0" once at Start and "RANGE?" must read back 0
    (printed 9-40 / 9-41), otherwise the run is refused.  RANGE takes
    no loop argument on a 340; the 350 form "RANGE 1,0" is a syntax
    error.  When unticked the heater is left exactly as found, which is
    what the 350 version always did (it never wrote to the Lakeshore).
  * The heater is never touched at stop, disconnect or close.
  * The sensor input stays selectable (A or B on a base Model 340;
    C and D only with the 3462 dual-input option card).  Default A.
  * RDGST? <input> is read with every sample (printed 9-41).  A
    non-zero status (invalid, old, under/over range, units zero or
    overrange) is logged with the sample and written into the Flags
    column, and such a sample is NOT used for the stop-window test, so
    an out-of-range 0.000 K can neither end the run nor pass as data.
  * The bus scan pre-selects the lab's 340 at IEEE address 19
    ("::19::", since 3 Sep 2026).  A hint only: the IDN check decides.
    12 and 15 are never auto-picked.
...
Not used, and not present on a 340: OUTMODE, TLIMIT, HTRSET, INNAME,
DFLD, "RANGE 1,n", "HTR? 1".  MODE is not sent (340 codes are 1/2/3).

===============================================================================
 PROGRAM:      AC R-T (T Sensing) - Keithley 6221 + SR830 + Lakeshore 340
 PURPOSE:      AC resistance against temperature, logged passively while
               ...
               TEMPERATURE
               -----------
               Apart from the optional RANGE 0 (heater off) at Start, this
               module only queries the Lakeshore 340. Whatever is driving
               the temperature -- a ramp started from the Lakeshore's own
               front panel, a PICA temperature control module, or a
               cryostat left to warm on its own -- keeps driving it, and
               the run simply records KRDG? against the AC resistance. The
               run ends when a VALID reading leaves the stop window, or
...
PROGRAM_VERSION = "1.0"
MODULE_NAME = "RT_AC_K6221_SR830_L340_T_Sensing_GUI.py"
PROGRAM_TITLE = "AC R-T (T Sensing) - Keithley 6221 + SR830 + Lakeshore 340"
FILE_SUFFIX = "AC_RT_Sensing"
...
DRIVE_HINT = (
    ...
    "Pick them with the AC I-V and frequency scan modules first.")
...
INFO_TEXT = (
    ...
    "  (voltage), Lakeshore 340 (read only)\n"
    ...
    "\n"
    "Nothing here writes to the Lakeshore 340 except an optional\n"
    "verified RANGE 0 (heater off) at Start. Drive the temperature\n"
    "from its front panel, from a PICA control module, or from\n"
    "the cryostat. Never *RST: it resets loop, setpoint and ramp.")
...
# -----------------------------------------------------------------------------
# --- LAKESHORE MODEL 340 THERMOMETRY ---
...
# wrong instrument is worse than not running at all: the address is checked by
# identity, never assumed, and only a MODEL340 reply is accepted because the
# one write this module can make (RANGE 0) must never reach another box.
#
# The lab's 340, 350, Cryocon 34 and Keithley 6221 all have factory address
# 12, so an address that "looks like the Lakeshore's" proves nothing. The 340
# was moved to IEEE address 19 on 3 Sep 2026; "::19::" is a hint for the bus
# scan only.
#
# Model 340 command set (User's Manual, Chapter 9):
#   *CLS      clear status registers (no *RST: printed 9-24, it resets loop,
#             setpoint and ramp to power-up values)
#   RANGE 0   Loop 1 heater off, no loop argument (printed 9-40); RANGE? (9-41)
#   KRDG? x   kelvin reading; RDGST? x reading status bits (printed 9-41)
# -----------------------------------------------------------------------------

LAKESHORE340_IDN_TOKENS = ("MODEL340", "MODEL 340")
...
def is_lakeshore340_idn(idn):
    """True only if a *IDN? reply came from a Lake Shore Model 340."""
    text = str(idn).upper().replace(' ', '')
    return any(tok.replace(' ', '') in text for tok in LAKESHORE340_IDN_TOKENS)
...
class Lakeshore340Monitor:
    """Passive thermometry from a Lake Shore Model 340.
    ...
    off. Everything else is a query. Whatever is driving the temperature --
    a Lakeshore ramp started from its own front panel, a cryostat, a dewar
    warming up on its own -- keeps driving it, and this module only watches.
    KRDG? returns Kelvin regardless of what the front panel is displaying,
    so there is no units trap here as there is on the Cryocon.
    """
    ...
    def __init__(self, visa_address, channel="A"):
        ...
        self.idn = self.instrument.query('*IDN?').strip()
        if not is_lakeshore340_idn(self.idn):
            try:
                self.instrument.close()
            finally:
                self.instrument = None
            raise ConnectionError(
                ...
                "'%s'. Refusing to send RANGE 0 to an unknown instrument. "
                "Scan the bus and use the 340's actual address (lab: ::19::)."
                % (visa_address, self.idn))

    def configure_for_monitoring(self, heater_off=True):
        """*CLS; optionally RANGE 0, verified with RANGE?.

        No *RST: on a 340 that resets loop, setpoint and ramp. Returns the
        heater range read back after configuration.
        """
        self.instrument.write('*CLS')
        time.sleep(0.2)
        if heater_off:
        ...
        if heater_off and rng != 0:
            raise RuntimeError(
                "Sent RANGE 0 but RANGE? reads back %d. Heater is NOT off. "
                "Check the front panel (Remote/Local) and try again." % rng)
        return rng
    ...
    def read_status(self):
        """RDGST? <channel> -> bit-weighted status (0 = good)."""
        return int(float(
            self.instrument.query('RDGST? %s' % self.channel).strip()))

    def shutdown(self):
        """Nothing to undo: the heater is left exactly as it is at stop."""
...
def thermometer_header_lines(thermometer, params):
    return [
        ...
        "# Lakeshore input channel: %s (KRDG?, Kelvin)" % thermometer.channel,
        "# Temperature control: NONE from this module. It only reads.",
        "# Heater at start: %s" % (
            "turned off (RANGE 0 sent, RANGE? = 0)"
            if params.get('heater_off', True) else "unchanged"),
        "# Reading status: RDGST? is read with every sample; a non-zero "
        "status is written into Flags and that sample is not used for the "
        "stop-window test.",
...
class ACResistanceTSensingGUI:
    PROGRAM_VERSION = PROGRAM_VERSION
    MODULE_NAME = MODULE_NAME
    DEFAULT_SOURCE_ADDRESS = "GPIB0::13::INSTR"
    DEFAULT_DETECTOR_ADDRESS = "GPIB0::8::INSTR"
    DEFAULT_THERMOMETER_ADDRESS = "GPIB1::19::INSTR"  # lab 340, re-addressed 3 Sep 2026
    LOGO_SIZE = 110
    ...
    def create_temperature_frame(self, parent):
        ...
        ttk.Label(
            frame,
            text=("Apart from the optional RANGE 0 below, this module never "
                  "writes to the %s. Drive the temperature from the "
                  "instrument's own front panel, from a PICA temperature "
                  "control module, or from the cryostat, and this run simply "
                  "records what the thermometer says."
                  % THERMOMETER_NAME),
            style='Sub.TLabel', wraplength=470, justify='left').grid(
            row=row, column=0, columnspan=3, padx=10, pady=(6, 8), sticky='w')
        row += 1
        ...
        self.heater_off_cb = ttk.Checkbutton(
            frame, text="Turn heater OFF at start (RANGE 0, verified with RANGE?)",
            variable=self.heater_off_var)
        self.heater_off_cb.grid(row=row, column=0, columnspan=3,
                                padx=10, pady=(2, 4), sticky='w')
        row += 1
    ...
    def _scan_for_visa_instruments(self):
        ...
        try:
            ...
            if resources:
                ...
                # Nothing is probed. The lab's 340 is at address 19
                # (3 Sep 2026; 12 is shared by the 350, the Cryocon and
                # the 6221), so ::19:: is pre-selected when present. Any
                # other address can still be picked by hand.
                preferred = [r for r in resources
                             if LAKESHORE340_ADDRESS_HINT in r]
                if preferred:
                    self.thermometer_cb.set(preferred[0])
                    self.log("Pre-selected %s for the Lakeshore 340 (lab "
                             "address 19). *IDN? must answer MODEL340 at "
                             "Connect." % preferred[0])
            else:
    ...
    def _connect_worker(self, addresses):
        ...
        try:
            with self.io_lock:
                thermometer = Lakeshore340Monitor(
    ...
    def _prepare_instruments(self):
        ...
        with self.io_lock:
            ...
            # Lakeshore 340: *CLS only (never *RST), then the optional
            # verified RANGE 0. A readback other than 0 raises and the
            # run ends here, before any data is taken.
            heater_off = params['temperature']['heater_off']
            rng = self.thermometer.configure_for_monitoring(heater_off)
        if heater_off:
            self.data_queue.put(
                ('log', "Lakeshore 340 heater turned OFF (RANGE 0 sent, "
                 "RANGE? = 0)."))
        else:
            self.data_queue.put(
                ('log', "Lakeshore 340 heater left as found: RANGE? = %d "
                 "(%s)." % (rng, 'off' if rng == 0 else 'ON')))
        if code:
            self.data_queue.put(
        ...
        try:
            ...
            while not self.stop_requested:
                with self.io_lock:
                    ...
                    reading_status = self.thermometer.read_status()
                status_text = describe_reading_status(reading_status)
                point = self._measure_point(frequency, current_rms)
                if status_text:
                    # Model 340 RDGST? (printed 9-41): the kelvin value that
                    # came with a non-zero status is not a real reading.
                    point['problems'].append(
                        "Lakeshore 340 RDGST? %d on input %s: %s. This "
                        "sample is not used for the stop-window test."
                        % (reading_status, self.thermometer.channel,
                           status_text))
                ...
                # is no end setpoint to arrive at. A sample with a non-zero
                # RDGST? (e.g. 0.000 K on an underrange input) must not end it.
                if status_text:
    ...
    def _safe_shutdown(self):
        ...
        thrown. The Lakeshore 340 heater is NOT touched here: whatever
        was driving the temperature keeps its heater state.
        """
        try:
            with self.io_lock:
