"""
Module: PPMS_Sync_Freq_Scan_L340_E4980A_GUI.py
Purpose: PASSIVE temperature-synchronized dielectric spectroscopy for
         ...
         Lakeshore 340 used STRICTLY READ-ONLY as the probe thermometer.
         Port of PPMS_Sync_Freq_Scan_E4980A_GUI.py to the Model 340
         command set.

This is the Lake Shore Model 340 sibling of
PPMS_Sync_Freq_Scan_E4980A_GUI.py, which stays exactly as it is for
Lakeshore 350 work (the Cryo-con sibling is
PPMS_Sync_Freq_Scan_CC34_E4980A_GUI.py). Only the thermometer differs:
the stability detection, plateau / resync logic, glitch filter, sweep
engine, hardening and data format are the same code.

What is different from the Model 350 version, and why
-----------------------------------------------------
  * *IDN? must contain MODEL340 (case-insensitive, spaces ignored) or the
    connection is REFUSED: the lab's 350, Cryocon 34 and Keithley 6221
    all share factory address 12, and every measured point of an
    overnight run is indexed by the temperature read here. The 350
    version only printed a warning when "350" was missing.
  * No *RST, ever. On a 340 "*RST sets controller parameters to
    power-up settings" (340 manual, printed 9-24): it resets the
    control loop, setpoint and ramp. Only *CLS is sent at connect, as
    the 350 version did.
  * Heater-off at Start is a checkbox, ON by default: "Turn heater OFF
    at start (RANGE 0)". RANGE takes no output number on a 340 — the
    350 form "RANGE 1,0" is a syntax error — so "RANGE 0" is sent once
    and "RANGE?" must read back 0 or Start is refused. (RANGE printed
    9-40, RANGE? 9-41.) Unticked, the heater is left exactly as found,
    like the 350 version. The heater is never touched at Stop or on a
    reconnect.
  * RDGST? <input> is read with every KRDG? sample (printed 9-41). A
    non-zero status (1 invalid, 2 old, 16 T under, 32 T over, 64 units
    zero, 128 units over) is logged with the reading and the sample is
    treated as an INVALID READ (GLITCH-1): it never enters a stability
    window, a plateau / resync detector or a plot, and the TempLog row
    is flagged '|GLITCH' exactly like a dropout.
  * HTR? would take no argument on a 340 (printed 9-33); this program
    never reads heater output, so it is not sent.
  * The bus scan still identifies every address by *IDN?, picks the
    thermometer by IDENTITY (MODEL340), and among several 340s prefers
    the lab address "::19::" (set 3 Sep 2026). Addresses 12 and 15 are
    never auto-picked.
  * Inputs A-D are offered (C and D exist only with the 3462 option
    card); default A, as before.
  * No 350-only command appears in this file: no OUTMODE, TLIMIT, DFLD,
    INNAME, HTRSET, no "RANGE 1,n" / "RANGE? 1" / "HTR? 1". MODE is not
    sent (the 350 version did not send it either).

Commands used (all verified against the Model 340 User's Manual, ch. 9):
  *IDN?          LSCI,MODEL340,<serial>,<firmware>
  *CLS           clear status registers (at connect / reconnect)
  RANGE 0        heater off, Loop 1 (optional, once at Start)   RANGE?  -> n
  KRDG? <input>  kelvin reading, +nnn.nnnE+n
  RDGST? <input> reading status bit weighting (0 = good)

The PPMS runs its own temperature sequence — this program never talks to
the PPMS and never sends a control command to the Lakeshore (no
RAMP/SETP/PID; the one optional write is "RANGE 0", heater off, at
Start). The sample sits on a custom probe with large
...
============================================================
v1.8 — DESYNC RECOVERY (Co-07 Fscan-at-Tstep forensic, 2026-07-23)
============================================================
Root cause fixed here: on the 2026-07-17..19 run the PPMS advanced
its .seq every 25 min while this scanner waited up to 90 min per
step — above ~205 K the scanner fell behind and every scan file was
labelled with the scanner's own (stale) setpoint. The sample was
always PPMS-controlled; only the labels were wrong.
  DATA-1   Scan files now record the MEASURED probe temperature:
           the per-row column is T_sample(K) (read right after each
           frequency point); the commanded setpoint appears ONLY in
           the '#' comment header (T_set = …). After the sweep a
           second TOP comment line (no footers — user decision) is
           inserted atomically with the median measured T and its
           deviation from T_set. A label can never again silently
           desync from reality.
  RESYNC-1 "Re-sync to PPMS on timeout" checkbox (OFF by default).
           When a stabilization timeout fires and the measured T sits
           closer to a LATER schedule setpoint than to the current
           one, the scanner jumps its step counter forward to that
           setpoint (skipped steps are logged as SKIPPED-resync in
           the TimingLog) and re-runs stability detection there —
           staying in lockstep with a PPMS that ran ahead. With the
           box off, the old proceed-anyway behavior stands, but see
           DESYNC-1.
  DESYNC-1 Unconditional desync alarm: whenever a stabilization wait
           ends (stable OR timeout) with the measured T further from
           the target than the guard (or 2×tol when guard is off),
           the log gets a loud MISLABEL-RISK banner + beep. No modal
           dialogs (unattended-run policy).
  GLITCH-1 Probe readings are validated before use: T ≤ 1 K (the
           Lake Shore 0.000-K dropout) or a jump > 20 K in one poll
           is treated as an INVALID READ — logged in the TempLog
           with phase '|GLITCH', never fed to the stability window,
           the drift fit, or the scan rows. A genuine step change is
           accepted once two consecutive readings agree (±2 K).
  AGNOS-1  "Setpoint-agnostic" mode (OFF by default): no schedule at
           all — the scanner watches the probe, detects each plateau
           by SELF-referenced flatness (peak-to-peak ≤ 2×tol at the
           window mean) + drift over the window, scans it, and labels
           the file with the measured MEDIAN temperature. A new
           plateau must sit ≥ Min ΔT (default 2 K) from the last
           scanned one. Nothing can desync because there is no step
           counter; the PPMS owns the plan entirely. Runs until Stop.
  DWELL-1  The sequence builder gains "Wait varies with T" (T:min
           pairs, same syntax as the tolerance table): per-step
           WAITFOR defaults become max(planner suggestion, table
           value), so the exported .seq under-waits nowhere — the
           run's own TimingLog suggested ~40 min above 200 K where
           the old flat 25 min dwell caused the runaway.
  RATE-2   Slope-matched default ramp rates from the measured probe
           response (Co-07 TempLog): 0.8 K/min below 120 K, 0.5 for
           120-150 K, 0.3 K/min above 150 K (the probe cannot follow
           faster there — the old 1-3 K/min only built lag). Tier
           picked by the warmer end of each ramp: direction-neutral,
           so DESCENDING (cooling) schedules get the same rates.
  TOL-3    "Safe" preset 20/25 K tolerances widened to 3.0/2.5 K
           (user decision — old 1.8/1.65 too tight near base T).
  TIME-1   Stabilization Timeout default 90 → 35 min. Design rule
           from the forensic: the timeout clock starts when the
           stability wait begins (right after the previous sweep),
           and its value must stay below ONE PPMS step period
           (ramp + WAITFOR) minus the scan time — the 90-min value
  ...
  COOL-1   Verified direction-neutral for cooling runs (descending
           ...
           smart-sleep warming detector cannot false-fire on a warm
           start (it must first dip below the arm temperature).

============================================================
v1.8-L340 — LAKE SHORE MODEL 340 SIBLING (4 Sep 2026)
============================================================
Derived from PPMS_Sync_Freq_Scan_E4980A_GUI.py v1.8-PPMS-Sync. That
file is unchanged and stays the Lakeshore 350 program; this one is its
Model 340 sibling, not a replacement. Everything below is forced by
the instrument (Lake Shore 340 User's Manual, chapter 9):

  L340-1  Identity check is a REFUSAL, not a warning: connect() raises
          unless *IDN? contains MODEL340. The 350 answers on the
          Cryo-con factory address and the 6221 shares it too; an
          address-based pick would drive the whole stability machine
          off the wrong instrument.
  L340-2  Heater-off at Start is an opt-out checkbox (ON by default).
          "RANGE 0" (no output number on a 340, printed 9-40) is sent
          once, "RANGE?" (9-41) must read back 0, else Start fails
          loudly. Unticked: nothing but queries is ever sent, exactly
          like the 350 version. Never re-sent on reconnect; never
          touched at Stop.
  L340-3  RDGST? (9-41) accompanies every KRDG?. A non-zero status is
          logged with the sample (throttled like GLITCH-1) and the
          sample is rejected as an INVALID READ, so a 0.000 K or a
          stale reading can never feed a stability window, the
          plateau detector or the resync logic. TempLog format is
          unchanged.
  L340-4  No *RST anywhere (9-24: resets loop, setpoint and ramp).
          Only *CLS, as before.
  L340-5  The VISA scan prefers the lab's "::19::" address among
          MODEL340 answers; identity still decides. Read/write
          terminations are "\n" (the 340 answers <CR><LF> + EOI).
  L340-6  Inputs A-D selectable (C, D need the 3462 card); default A.
"""
...
TEMP_LOG_S = 0.10            # one interleaved KRDG? + RDGST? per frequency point
...
TOL_TABLE_PRESETS = {
    ...
    # TOL-3 (2026-07-23): 20/25 K widened to 3.0/2.5 (user decision) —
    # the probe offset near base T plus PPMS settling wander made the
    # old 1.8/1.65 too tight in practice.
    "Safe (recommended)":
...
# (the Co-07 run logged 39 stray 0.000 K reads plus a few non-zero
# garbage spikes — brief Lake Shore glitches, never real temperature)
# ============================================================
...
# ============================================================
# DWELL-1: temperature-dependent PPMS wait (WAITFOR) table
...
# ============================================================
# BACKEND: Lakeshore 340 as READ-ONLY probe thermometer
# (pattern of pica/lakeshore/T_Sensing_L340_GUI.py, inlined so this
#  module stays self-contained; interface — connect / reconnect /
#  get_temperature / shutdown — identical to the 350 parent so the
#  worker thread below is byte-for-byte the parent module's)
# ============================================================
...
def is_lakeshore340_idn(idn):
    """True if a '*IDN?' reply came from a Lake Shore Model 340."""
    text = str(idn).upper().replace(' ', '')
    return any(tok.replace(' ', '') in text for tok in LAKESHORE340_MODEL_TOKENS)


class Probe_Thermometer_Backend:
    """Read-only probe thermometer on a Lake Shore Model 340.

    The only commands ever sent are *CLS, *IDN?, KRDG? and RDGST?, plus
    ONE optional "RANGE 0" (heater off, Loop 1) at Start when the
    operator has left the heater-off checkbox ticked. The PPMS owns
    temperature control entirely. No *RST (on a 340 it resets loop,
    setpoint and ramp to power-up settings), no RAMP, SETP or PID.

    L340-1: connect() refuses anything whose *IDN? is not a MODEL340.
    """
    ...
    def __init__(self):
        ...
        self.idn = ""
        self.last_reading_status = 0   # L340-3: RDGST? of the last sample
        self.rm = None
        if pyvisa:
            try:
    ...
    def connect(self, visa_address):
        ...
        self.idn = idn
        if not is_lakeshore340_idn(idn):
            # L340-1: refuse, do not warn. Every measured point is indexed
            # by the temperature read here.
            try:
            ...
            raise ConnectionError(
                ...
                "Shore Model 340. Refusing to log its readings as the sample "
                "temperature (and refusing to send RANGE 0 to it). Scan the "
                "bus and pick the address that answers as MODEL340 "
                f"(the lab's 340 is at {LAKESHORE340_ADDRESS_HINT}).")
        return idn
    ...
    def reconnect(self):
        ...
        still refuses anything that is not a MODEL340). The heater is
        NOT touched here: RANGE 0 is a Start-time action only."""
        try:
            self.shutdown()
        except Exception as e:
    ...
    # -- L340-2: optional heater-off at Start --

    def configure_for_monitoring(self, heater_off=True):
        """Optionally turn the Loop 1 heater off, verified.

        heater_off=True: send "RANGE 0" once and require "RANGE?" == 0.
        heater_off=False: send nothing; the heater stays as found.
        Returns the heater range read back afterwards (0 = off).
        """
        ...
        if heater_off and rng != 0:
            raise RuntimeError(
                ...
                "Check the front panel (Remote/Local) and try again, or untick "
                "'Turn heater OFF at start' to leave it as it is.")
        return rng
    ...
    def get_reading_status(self, channel):
        ...
        return int(float(self.lakeshore.query(f"RDGST? {channel}").strip()))
    ...
    def get_temperature(self, channel, retries=2):
        ...
        L340-3: RDGST? is read with every sample and left in
        self.last_reading_status for the worker to judge the reading.
        The temperature is returned as the 340 reports it (a flagged
        reading is often 0.000 K, which GLITCH-1 rejects on its own)."""
        last_err = None
        for attempt in range(retries + 1):
            try:
                temp = float(
                    self.lakeshore.query(f"KRDG? {channel}").strip())
                self.last_reading_status = self.get_reading_status(channel)
                return temp
            except Exception as e:
                last_err = e
                if attempt < retries:
    ...
    def shutdown(self):
        ...
        the heater state is left exactly as it is."""
        if self.lakeshore:
            try:
                self.lakeshore.close()
            except Exception as e:
...
# ============================================================
class PPMSSyncGUI:
    PROGRAM_VERSION = "1.8-PPMS-Sync-L340"  # Lake Shore 340 sibling of v1.8
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(
            "PPMS-Sync Dielectric Spectroscopy (Lakeshore 340) "
        ...
        self.log(f"PPMS-Sync GUI (L340) v{self.PROGRAM_VERSION} initialized. "
                 "Lakeshore 340 is READ-ONLY (KRDG?/RDGST? only, plus one "
                 "optional RANGE 0 at Start) — the PPMS owns all "
                 "temperature control.")
    ...
    def _create_stability_panel(self, parent, row):
        ...
        self._create_grid_entry(frame, "Target guard (±K, 0=off):",
                                "guard", "2.0", 3, 3)
        # TIME-1: 35 min ≈ one slope-matched PPMS step period minus the
        # scan — a single timeout can never leave the scanner a full
        # step behind (the old 90 min default let it fall 3+ steps
        # behind per timeout on the Co-07 run).
        ...
        # RESYNC-1: jump the step counter forward on a diverged timeout
        # (OFF by default — new behavior is opt-in).
        ...
        self.chk_resync.grid(row=9, column=0, columnspan=6,
                             sticky="w", padx=10, pady=(4, 0))

        # AGNOSTIC-1: setpoint-free plateau follower (OFF by default).
        self.var_agnostic = tk.BooleanVar(value=False)
        self.chk_agnostic = ttk.Checkbutton(
            frame, text="Setpoint-agnostic: scan EVERY plateau the PPMS "
                        "makes (schedule optional; label = measured "
                        "median).  Min ΔT between scans (K):",
            variable=self.var_agnostic)
        self.chk_agnostic.grid(row=10, column=0, columnspan=5,
                               sticky="w", padx=10, pady=(2, 0))
        self.agn_dt_entry = ttk.Entry(frame, width=6)
        self.agn_dt_entry.insert(0, "2.0")
        self.agn_dt_entry.grid(row=10, column=5, sticky="w",
                               padx=(2, 10), pady=(2, 0))

        ttk.Button(frame, text="Apply Live Updates",
                   command=self._send_live_updates
                   ).grid(row=11, column=0, columnspan=6, sticky="ew",
                          padx=10, pady=(2, 6))
    ...
    def _create_thermo_panel(self, parent, row):
        ...
        ttk.Label(frame, text="Input Ch:").grid(row=1, column=0, sticky="w",
                                                padx=10, pady=5)
        # A, B on a base Model 340; C, D only with the 3462 option card.
        ...
        # L340-2: heater-off at Start. ON by default — the probe is
        # supposed to follow the PPMS freely, and a 340 keeps whatever
        # heater range was last set through its keypad.
        self.var_heater_off = tk.BooleanVar(value=True)
        self.heater_off_chk = ttk.Checkbutton(
            frame,
            text="Turn heater OFF at start (RANGE 0, verified with RANGE?)",
            variable=self.var_heater_off)
        self.heater_off_chk.grid(row=2, column=0, columnspan=3, sticky="w",
                                 padx=10, pady=(2, 0))
        ttk.Label(frame,
                  text="Only *IDN?/*CLS/KRDG?/RDGST? are ever sent, plus "
                       "the one optional RANGE 0 above — no ramp, setpoint "
                       "or PID command exists in this program. Heater is "
                       "never touched at Stop.",
                  font=("Segoe UI", 8, "italic"), wraplength=420
    ...
    def _build_timing_tab(self, parent):
        ...
        # --- DWELL-1: temperature-dependent WAITFOR floor (Co-07: the
        # flat 25-min dwell above ~205 K caused the label runaway; the
        # run's own TimingLog suggested ~40 min there) ---
        dwl = ttk.Frame(parent)
        dwl.pack(side="top", fill="x", padx=8, pady=(0, 2))
        self.var_dwell_table = tk.BooleanVar(value=True)
        self.chk_dwell_table = ttk.Checkbutton(
            dwl, text="Wait varies with T (T:min pairs — floor under "
                      "each row's wait; hold below/above the table, "
                      "interpolate between):",
            variable=self.var_dwell_table)
        self.chk_dwell_table.pack(side="left")
        self.dwell_table_entry = ttk.Entry(dwl, width=30)
        self.dwell_table_entry.insert(0, DWELL_TABLE_DEFAULT)
        self.dwell_table_entry.pack(side="left", padx=(4, 0))
        ...
        self.seq_controls = [self.seq_load_btn, self.seq_init_entry,
                             ...
                             self.seq_export_btn,
                             self.chk_dwell_table, self.dwell_table_entry]
    ...
    def _scan_for_visa(self):
        ...
        try:
            for res in rm.list_resources():
                ...
                label = f"{res}  ->  {idn}"
                found.append(label)
                # L340-1/L340-5: identity, never address. Among several
                # MODEL340 answers the lab's ::19:: wins; 12 and 15 are
                # never picked by address alone.
                if is_lakeshore340_idn(idn) and (
                        ls_pick is None
            ...
            else: self.log("No MODEL340 answered *IDN?. Pick the address by "
                           "hand only if you are sure; Start refuses anything "
                           "that does not identify as a Model 340.")
    ...
    def start_sequence(self):
        ...
        try:
            ...
            return
        if not self.schedule and not self.var_agnostic.get():
            messagebox.showwarning(
                "Empty Schedule",
                "Add at least one setpoint (or enable Setpoint-agnostic "
                "mode, which needs no schedule)."); return
        ...
        self._pending_progress = None
        # AGNOSTIC-1: with no schedule the number of plateaus is unknown
        # — the bar then tracks a single sweep at a time.
        self.progress["maximum"] = (
            max(len(self.schedule), 1) * len(self.sweep_frequencies)
            if not self.var_agnostic.get() else len(self.sweep_frequencies))
    ...
    def _validate_params(self):
        ...
        p = {
            ...
            "heater_off": bool(self.var_heater_off.get()),   # L340-2
            ...
            "agnostic": self.var_agnostic.get(),    # AGNOSTIC-1
    ...
    def set_ui_state(self, running: bool):
        ...
        # RESYNC-1 / AGNOSTIC-1: read once at Start
        self.chk_resync.config(state=st)
        self.chk_agnostic.config(state=st)
        self.agn_dt_entry.config(state=st)
        ...
        # L340-2: the heater-off choice is read once at Start.
        self.heater_off_chk.config(state=st)
    ...
    def _seq_default_rate(self, target, prev):
        ...
        rate. RATE-2 (2026-07-23): SLOPE-MATCHED to the measured probe
        response of the Co-07 reference run — the probe follows the
        block at ~0.8-0.95 K/min below 120 K but only ~0.25-0.3 K/min
        above 150 K; commanding the block faster (the old 1-3 K/min)
        only builds probe lag that the dwell then has to absorb. The
        tier is chosen by the WARMER end of the ramp, so warming and
        cooling ladders get the same (direction-neutral) rate. Purely
        a starting suggestion — every cell is editable."""
        if prev is None:
            return None
        hi = max(float(target), float(prev))
        if hi < 120.0:
            rate = 0.8
        elif hi <= 150.0:
            rate = 0.5
    ...
    def _fill_timing_tab(self, targets, suggest_s, keep_edits=True):
        ...
        # DWELL-1: per-target floor from the wait-vs-T table.
        dwell_table = self._dwell_table_or_none()
        rows = []
        prev = None
        for t in targets:
            ...
            w = wait_min if dwell_table is None \
                else max(wait_min, dwell_from_table(dwell_table, t))
            rows.append({"target": t, "dT": dT, "rate": rate,
                         "wait": w, "status": "planned"})
            prev = t
        self.seq_rows = rows
    ...
    def _dwell_table_or_none(self):
        """DWELL-1: parsed wait-vs-T table (T:min pairs, tolerance-table
        syntax), or None when the box is off / entry unparsable (a bad
        table only loses the floor — it never blocks planning)."""
        if not getattr(self, "var_dwell_table", None) \
                or not self.var_dwell_table.get():
            return None
        try:
            return parse_tol_table(self.dwell_table_entry.get())
        except (ValueError, tk.TclError) as e:
            self.log(f"Wait-vs-T table ignored (parse error: {e}).")
            return None
    ...
    def _export_seq(self):
        ...
        try:
            ...
            return
        # TIME-1: matched-timeout advice from THIS plan. Rule (Co-07
        # forensic): the scanner's stabilization Timeout must not exceed
        # one PPMS step period (ramp + WAITFOR) minus the scan, or a
        # single timeout leaves the scanner a full step behind.
        self._update_scan_estimate()
        per_step_s = []
        prev_t = None
        for target, rate, wait_s in steps:
            if prev_t is not None and rate > 0:
                per_step_s.append(abs(target - prev_t) / rate * 60.0
                                  + wait_s)
            prev_t = target
        if per_step_s:
            shortest = min(per_step_s)
            rec_min = max(5.0, (shortest - self._scan_est_s - 120.0) / 60.0)
            self.log(f"Matched-timeout advice: shortest PPMS step in this "
                     f"plan is {fmt_hms(shortest)} (ramp + wait). Set the "
                     f"scanner's stabilization Timeout ≤ {rec_min:.0f} min "
                     f"(= step − scan {fmt_hms(self._scan_est_s)} − 2 min) "
                     f"so one timeout can never leave the scanner a full "
                     f"step behind the PPMS.")
    ...
    def _hardware_worker_loop(self):
        ...
        try:
            ...
            # L340-2: the one optional write of this program, at Start
            # only. Verified by RANGE?; a failure aborts before any data
            # file exists (Start is attended by definition).
            rng = self.thermo_backend.configure_for_monitoring(
                heater_off=self.params["heater_off"])
            if self.params["heater_off"]:
                self._put_gui_msg("log",
                    text="Lakeshore 340 heater turned OFF (RANGE 0 sent, "
                         "RANGE? = 0). It is not touched again.")
            else:
                self._put_gui_msg("log",
                    text=f"Lakeshore 340 heater left as found: RANGE? = "
                         f"{rng} ({'off' if rng == 0 else 'ON'}).")
            # First reading with its RDGST? status, so a wrong or empty
            # input is named here, by letter, instead of poisoning a
            # night of stability waits.
            t0 = self.thermo_backend.get_temperature(self.params["channel"])
            st0 = self.thermo_backend.last_reading_status
            st0_txt = describe_reading_status(st0)
            self._put_gui_msg("log",
                text=f"Input {self.params['channel']} reads {t0:.3f} K "
                     f"[RDGST {st0}: {st0_txt or 'OK'}]")
            if st0_txt:
                self._put_gui_msg("log",
                    text=f"⚠️ Input {self.params['channel']} status is not "
                         f"clean ({st0_txt}). Check the sensor / input "
                         "letter — such samples are rejected as invalid "
                         "reads for the whole run.")
            ...
            if p["sleep_enabled"] and p["initial_sleep_min"] > 0 \
                    and self.is_running:
                first_target = (self.schedule[0] if self.schedule
                                else float("nan"))
                if self.schedule:
            ...
            if p.get("agnostic") and self.is_running:
                # AGNOSTIC-1: no schedule — follow PPMS plateaus.
                self._run_agnostic_loop(initial_sleep_used_s)
                n_steps = 0   # the schedule loop below is bypassed
            i = 0
            first_step = True
            while self.is_running and i < n_steps:
                ...
                sleep_used_s = initial_sleep_used_s if first_step else 0.0
                ...
                if stab_outcome == "timeout":
                    ...
                    # DESYNC-1: loud mislabel-risk alarm whenever the
                    # sample sits outside the guard around the target.
                        ...
                        if p.get("resync"):
                            ...
                            if j is not None:
                                for k in range(i, j):
                                    ...
                                    self._put_gui_msg(
                                        "timing_row", index=k,
                                        target=self.schedule[k],
                                        sleep_s=0.0, settle_s=0.0,
                                        scan_s=0.0, suggest_s=0.0,
                                        status="SKIPPED (resync)")
                                i = j
    ...
    # ------------------------------------------------------------
    # RESYNC-1: catch up with a PPMS that ran ahead
    # ------------------------------------------------------------
    def _resync_step(self, i):
        """Called on a diverged stabilization timeout at step i. Returns
        the index j > i of the LATER schedule setpoint the measured
        temperature fits best — only if it fits strictly better than
        the current step's setpoint — else None (no jump)."""
        T = self._last_temp
    ...
    # ------------------------------------------------------------
    # AGNOSTIC-1: setpoint-free plateau follower
    # ------------------------------------------------------------
    def _run_agnostic_loop(self, initial_sleep_used_s):
        """No schedule, no step counter, nothing to desync: the PPMS
        owns the temperature plan entirely; this loop just detects each
        plateau (self-referenced flatness + drift over the window),
        labels the sweep with the MEDIAN measured temperature and waits
        for the next plateau (which must sit at least agn_min_dT away
        from the last scanned one). Runs until Stop."""
        p = self.params
        n_scan = 0
        last_label = None
        first = True
        self._put_gui_msg("log",
            text="AGNOSTIC MODE: no setpoint schedule — every detected "
                 "plateau (peak-to-peak ≤ 2×Tol AND |drift| ≤ limit over "
                 f"the {p['window_min']:g}-min window) is scanned and "
                 "labelled with the measured MEDIAN temperature. Next "
                 f"plateau must differ by ≥ {p['agn_min_dT']:g} K. "
                 "Runs until Stop.")
        while self.is_running:
            step_start_dt = datetime.now()
            outcome, label_T, settle_s = self._wait_for_plateau(last_label)
            if outcome != "plateau" or not self.is_running:
                break
            n_scan += 1
            self._put_gui_msg("log",
                text=f"Plateau #{n_scan} detected at {label_T:.3f} K "
                     f"after {fmt_hms(settle_s)} — starting sweep.")
            self._put_gui_msg("status",
                text=f"SCANNING AT {label_T:.2f} K",
                color=self.CLR_ACCENT_GREEN)
            self._put_gui_msg("beep")
            self._put_gui_msg("scan_reset", target=label_T)
            _, scan_s, drift_flag = self._run_frequency_sweep(
                label_T, 0, len(self.sweep_frequencies))
            self._write_timing_row(
                n_scan - 1, label_T, step_start_dt,
                initial_sleep_used_s if first else 0.0,
            ...
            last_label = label_T
            self._put_gui_msg("log",
                text=f"Sweep done at {label_T:.3f} K. Watching for the "
                     f"next plateau (≥ {p['agn_min_dT']:g} K away)…")

    def _wait_for_plateau(self, last_label):
        """AGNOSTIC-1 stability wait: SELF-referenced — no target. The
        ...
        to get wrong there is nothing to force. 'Skip Step' scans the
        current window immediately.
        Returns (outcome, label_T, wait_s); outcome 'plateau'|'stopped';
        label_T = median of the window."""
        p = self.params
        self._worker_phase = "WAIT_STABLE"
        window = deque()
        phase_start = time.time()
            ...
            while self.is_running:
                ...
                if now - last_status > 3.0:
                    ...
                    if m is not None:
                        ...
                        self._put_gui_msg("status",
                            text=(f"PLATEAU WATCH ~{m['med']:.2f} K | "
                                  f"window {fill:.0f}% | "
                                  f"p-p {m['pkpk']:.3f} K | "
                                  f"drift {m['drift']:+.3f} K/min"
                                  + extra),
    ...
    def _log_temperature_point(self, target, measuring_flag):
        ...
        # never enter the stability window and never reach the plot.
        # L340-3: a non-zero RDGST? makes the sample invalid too, and
        # the status is named in the log line with the reading.
        rdgst = getattr(self.thermo_backend, "last_reading_status", 0)
        rdgst_txt = describe_reading_status(rdgst)
        valid = self._validate_probe_reading(temp) and not rdgst_txt
        if valid:
            self._last_temp = temp
        else:
            self._glitch_candidate = None if rdgst_txt \
                else self._glitch_candidate
            self._glitch_total += 1
            if self._glitch_total <= 5 or self._glitch_total % 25 == 0:
                why = (f" [RDGST {rdgst}: {rdgst_txt}]" if rdgst_txt
                       else "")
                self._put_gui_msg("log",
                    text=f"⚠️ INVALID PROBE READ #{self._glitch_total}: "
                         f"{temp:.3f} K{why} (last valid "
        ...
        if valid and temp > 340.0 and not self._overtemp_warned:
        ...
        if valid:
            self._put_gui_msg("temp_point", t=elapsed, temp=temp,
                              target=target, measuring=measuring_flag)
