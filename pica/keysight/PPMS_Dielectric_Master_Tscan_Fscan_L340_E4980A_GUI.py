"""
Module: PPMS_Dielectric_Master_Tscan_Fscan_L340_E4980A_GUI.py
Purpose: PPMS Dielectric Master — fully unattended multi-day protocol
         ...
         E4980A, with the Lakeshore 340 used STRICTLY READ-ONLY as the
         probe thermometer. The PPMS runs its own MultiVu sequence;
         this program never talks to the PPMS and never sends a
         control command to the Lakeshore (no RAMP/SETP/PID; the one
         optional write is "RANGE 0", heater off, at Start).
         Port of PPMS_Dielectric_Master_Tscan_Fscan_E4980A_GUI.py to the
         Model 340 command set.

This is the Lake Shore Model 340 sibling of
PPMS_Dielectric_Master_Tscan_Fscan_E4980A_GUI.py, which stays exactly as
it is for Lakeshore 350 work (the Cryo-con sibling is
PPMS_Dielectric_Master_Tscan_Fscan_CC34_E4980A_GUI.py). Only the
thermometer differs: the protocol, the phase detector, the sequence
generator, the hardening and the data format are the same code.
...
-----------------------------------------------------
  * *IDN? must contain MODEL340 (case-insensitive, spaces ignored) or the
    ...
    all share factory address 12, and every measured point of a
    multi-day run is indexed by the temperature read here. The 350
  ...
  * RDGST? <input> is read with every KRDG? sample (printed 9-41). A
    ...
    window, a turnaround detector or a plot, and the TempLog row is
    flagged '|GLITCH' exactly like a dropout.
...
============================================================
v1.5-L340 — LAKE SHORE MODEL 340 SIBLING (3 Sep 2026)
============================================================
...
is its Model 340 sibling, not a replacement. Everything below is
forced by the instrument (Lake Shore 340 User's Manual, chapter 9):
  ...
  L340-1  Identity check is a REFUSAL, not a warning: connect() raises
          ...
          address-based pick would drive the whole phase machine off
  ...
  L340-3  RDGST? (9-41) accompanies every KRDG?. A non-zero status is
          ...
          stale reading can never feed a stability window or a
          turnaround detector. TempLog format is unchanged.
...
class Probe_Thermometer_Backend:
    ...
    def connect(self, visa_address):
        ...
        self.visa_address = visa_address   # kept for reconnect()
    ...
    def reconnect(self):
        """Close and re-open the session from the stored address.
    ...
    def get_temperature(self, channel, retries=2):
        """Retry transient VISA glitches (device clear + backoff) so one
        failed query at 3 a.m. does not abort an overnight run.
...
# ============================================================
class PPMSMasterGUI:
    PROGRAM_VERSION = "1.5-L340"   # Lake Shore 340 sibling of v1.5
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(
            "PPMS Dielectric Master (Lakeshore 340): Tscan + Fscan "
        ...
        self.log(f"PPMS Dielectric Master (L340) v{self.PROGRAM_VERSION} "
                 "initialized. Lakeshore 340 is READ-ONLY (KRDG?/RDGST? "
                 "only, plus one optional RANGE 0 at Start) — the PPMS "
                 "owns all temperature and field control.")
    ...
    def _create_thermo_panel(self, parent, row):
        ...
        cb.grid(row=2, column=0, columnspan=3, sticky="w", padx=10,
                pady=(2, 0))
        self._prerun_simple.append(cb)
        ttk.Label(frame,
    ...
    def _scan_for_visa(self):
        ...
        try:
            ...
            if ls_pick:
                ...
                self.log(f"Lakeshore 340 auto-selected: {ls_pick}")
            else:
                self.log("No MODEL340 answered *IDN?. Pick the address by "
    ...
    def _hardware_worker_loop(self):
        ...
        try:
            ...
            # input is named here, by letter, instead of poisoning days
            # of protocol.
