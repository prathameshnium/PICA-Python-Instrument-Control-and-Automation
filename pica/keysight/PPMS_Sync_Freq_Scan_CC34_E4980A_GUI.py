"""
Module: PPMS_Sync_Freq_Scan_CC34_E4980A_GUI.py
Purpose: PASSIVE temperature-synchronized dielectric spectroscopy for
         ...
         Cryo-con Model 34 used STRICTLY READ-ONLY as the probe
         thermometer.

This is the Cryo-con sibling of PPMS_Sync_Freq_Scan_E4980A_GUI.py, which
stays exactly as it is for Lakeshore 350 work. Only the thermometer
differs: the scan engine, the stability logic, the timing planner, the
sequence generator and the data format are the same code, and the two
programs produce interchangeable files.

The PPMS runs its own temperature sequence — this program never talks to
the PPMS and never sends a single control command to the Cryocon (no
STOP/CONTROL/loop/setpoint/heater). The sample sits on a custom probe with large
...
============================================================
v1.8-CC34 — CRYO-CON MODEL 34 SIBLING (29 Aug 2026)
============================================================
Derived from PPMS_Sync_Freq_Scan_E4980A_GUI.py v1.8. That file is
unchanged and stays the Lakeshore 350 program; this one is its
Cryo-con sibling, not a replacement. Everything below is forced by
the instrument:
  ...
  CC34-1  Thermometry is a Cryo-con Model 34. The reading command is
          ...
          three into a dead worker thread.
  CC34-2  The input channel is verified at Start. INPUT? reports in
          each channel's own display units, so a channel left in C
          or F would log wrong numbers against every point of a
          multi-day run. 'INPUT <ch>:UNITS?' must answer K, and the
          channel must return a live reading, before the run begins.
  CC34-3  The instrument is chosen by identity, never by address.
          ...
          factory address — so an address-based pick would index a
          whole run by the wrong instrument's temperature. Connect
  ...
  CC34-5  A sensor-status reply is an INVALID READING, not a comm
          ...
          '|GLITCH' in the TempLog and the run carries on — where
          raising would have sent the retry-forever reconnect loop
          spinning for the rest of the night on a fault reconnecting
          cannot cure.
...
# (read-only; inlined so this module stays self-contained)
# ============================================================
#
...
# ============================================================
class PPMSSyncGUI:
    PROGRAM_VERSION = "1.8-PPMS-Sync-CC34"  # Cryo-con 34 sibling of v1.8
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(
            "PPMS-Sync Dielectric Spectroscopy (Cryo-con 34) "
        ...
        self.log(f"PPMS-Sync CC34 GUI v{self.PROGRAM_VERSION} "
                 "initialized. Cryo-con Model 34 is READ-ONLY (INPUT? "
                 "only) — the PPMS owns all temperature control.")
    ...
    def _scan_for_visa(self):
        ...
        try:
            ...
            if ls_pick: self.ls_cb.set(ls_pick); self.log(f"Cryocon auto-selected: {ls_pick}")
    ...
    def _validate_params(self):
        ...
        if not p["thermo_visa"]: raise ValueError("Select the Cryocon VISA.")
