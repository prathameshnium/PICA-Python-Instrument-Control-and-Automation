"""
===============================================================================
 PROGRAM:      AC I-V Sweep - Keithley 6221 + 197A
 PURPOSE:      AC I-V: sweep the current amplitude at one fixed frequency
               and watch the voltage follow it. A straight line through the
               origin is an ohmic contact; a resistance that drifts with
               current is self heating or a rectifying junction, and it says
               which current the temperature scans should be run at.
...
PROGRAM_VERSION = "1.0"
MODULE_NAME = "IV_AC_K6221_K197A_GUI.py"
PROGRAM_TITLE = "AC I-V Sweep - Keithley 6221 + 197A"
FILE_SUFFIX = "AC_IV_197A"

# One line for the file header, saying what the run actually was.
MEASUREMENT_DESCRIPTION = (
    "AC I-V, current amplitude swept at a fixed frequency")

# The hint above the drive boxes, and the axis labels the detector
# decides.
DRIVE_HINT = (
    "The frequency is held for the whole sweep and the current "
    "amplitude is stepped. Both sweep boxes are in A rms. Start high: "
    "the 197A needs a voltage it can actually resolve.")
VOLTAGE_AXIS_LABEL = "V rms"
SUB_AXIS_KEY = "voltage"
SUB_AXIS_LABEL = "V rms (V)"

INFO_TEXT = (
    "Program: AC I-V sweep (four probe, magnitude only)\n"
...
DATA_COLUMNS = ("Timestamp,Elapsed (s),Frequency set (Hz),"
...
class ACCurrentSweepGUI:
    ...
    def create_drive_frame(self, parent):
        ...
        row = self._add_entry(frame, row, 'frequency', "Frequency (Hz)",
                              "133.0", "held fixed for the whole sweep")
        ...
        row = self._add_entry(frame, row, 'sweep_start',
                              "Start current (A rms)", "1e-6")
        row = self._add_entry(frame, row, 'sweep_stop',
                              "Stop current (A rms)", "1e-4")
        row = self._add_entry(frame, row, 'sweep_points', "Points", "11")

        self.log_spacing_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            frame, text="Logarithmic spacing",
            variable=self.log_spacing_var).grid(
            row=row, column=0, columnspan=3, padx=10, pady=(4, 10), sticky='w')
    ...
    def create_graph_frame(self, parent):
        ...
        self.line_sub, = self.ax_sub.plot(
            ...
            linestyle='-')
        self.ax_main.set_title("Voltage vs. current", fontweight='bold')
        self.ax_main.set_xlabel("Current amplitude (A rms)")
        self.ax_main.set_ylabel("%s (V)" % VOLTAGE_AXIS_LABEL)
        self.ax_sub.set_title("Resistance vs. current", fontweight='bold')
        self.ax_sub.set_xlabel("Current amplitude (A rms)")
        self.ax_sub.set_ylabel("R (Ohm)")
    ...
    def _connect_worker(self, addresses):
        ...
        self.action_queue.put(
            ('ok', None, (source, detector, None), self._on_connected))
    ...
    def _read_parameters(self):
        ...
        start = number('sweep_start', "Start current")
        stop = number('sweep_stop', "Stop current")
        points = int(number('sweep_points', "Points"))
        if points < 1:
            raise ValueError("A sweep needs at least one point.")
        spacing = log_points if self.log_spacing_var.get() else linear_points
        # The boxes are in A rms because that is the convention the voltage is
        # in; the 6221 is programmed in peak, so the conversion happens here
        # and exactly once.
        currents_rms = spacing(start, stop, points)
        setpoints = [(frequency, peak_from_rms(value))
                     for value in currents_rms]
        ...
        drive_params = {
            ...
            'frequency': frequency,
            'drive_description':
                "%d-point current sweep, %.4E to %.4E A rms at %.4f Hz"
                % (points, currents_rms[0], currents_rms[-1], frequency),
        }
    ...
    def _run_worker(self):
        """Walk the setpoints once. The only thread that touches instruments."""
        params = self.params
        first_point = True
        try:
            self.data_queue.put(
            ...
            for index, (frequency, current_peak) in enumerate(setpoints):
                ...
                self._emit_point(
                    point, frequency, current_peak, current_rms, None)

            self.data_queue.put(
                ('done', "Stopped." if self.stop_requested else "Sweep "
                 "finished."))
        except Exception as exc:
    ...
    def _record_point(self, point):
        ...
        self.data_storage['x'].append(point['current_rms'])
        self.data_storage['y'].append(point['voltage'])
        self.data_storage['sub_x'].append(point['current_rms'])
        self.data_storage['sub_y'].append(point['resistance'])
        self.line_main.set_data(
