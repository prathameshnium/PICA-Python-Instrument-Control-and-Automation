"""
===============================================================================
 PROGRAM:      AC Frequency Scan - Keithley 6221 + 197A
 PURPOSE:      AC frequency scan: sweep the drive frequency at one fixed
               current amplitude. A four-probe resistance that is flat in
               frequency is a resistance; one that is not is telling you about
               cable capacitance, contact impedance, or the limits of the
               measurement itself.
...
PROGRAM_VERSION = "1.0"
MODULE_NAME = "Frequency_Scan_K6221_K197A_GUI.py"
PROGRAM_TITLE = "AC Frequency Scan - Keithley 6221 + 197A"
FILE_SUFFIX = "AC_Fscan_197A"

# One line for the file header, saying what the run actually was.
MEASUREMENT_DESCRIPTION = (
    "AC frequency scan at a fixed current amplitude")

# The hint above the drive boxes, and the axis labels the detector
# decides.
DRIVE_HINT = (
    "The current amplitude is held for the whole sweep and the "
    "frequency is stepped. Outside the 197A AC volts band the meter "
    "rolls the signal off, so a scan that leaves it is measuring the "
    "meter and not the sample.")
VOLTAGE_AXIS_LABEL = "V rms"
SUB_AXIS_KEY = "voltage"
SUB_AXIS_LABEL = "V rms (V)"

INFO_TEXT = (
    "Program: AC frequency scan (four probe, magnitude only)\n"
    ...
    "\n"
    "A roll-off at the top of this scan is the METER's passband\n"
    "before it is the sample's. Confirm the shape against the\n"
    "SR830 module before it is read as physics.")
...
class ACFrequencyScanGUI:
    ...
    def create_drive_frame(self, parent):
        ...
        row = self._add_entry(frame, row, 'sweep_start',
                              "Start frequency (Hz)", "50.0")
        row = self._add_entry(frame, row, 'sweep_stop',
                              "Stop frequency (Hz)", "5000.0")
        row = self._add_entry(frame, row, 'sweep_points', "Points", "21")
    ...
    def create_graph_frame(self, parent):
        ...
        self.line_sub, = self.ax_sub.plot(
            ...
            linestyle='-')
        self.ax_main.set_title("Resistance vs. frequency", fontweight='bold')
        self.ax_main.set_xlabel("Frequency (Hz)")
        self.ax_main.set_ylabel("R (Ohm)")
        self.ax_main.set_xscale('log')
        self.ax_sub.set_title("%s vs. frequency" % SUB_AXIS_LABEL,
                              fontweight='bold')
        self.ax_sub.set_xlabel("Frequency (Hz)")
        self.ax_sub.set_ylabel(SUB_AXIS_LABEL)
        self.ax_sub.set_xscale('log')
    ...
    def _read_parameters(self):
        ...
        start = number('sweep_start', "Start frequency")
        stop = number('sweep_stop', "Stop frequency")
        points = int(number('sweep_points', "Points"))
        if points < 1:
            raise ValueError("A sweep needs at least one point.")
        spacing = log_points if self.log_spacing_var.get() else linear_points
        frequencies = spacing(start, stop, points)
        setpoints = [(value, current_peak) for value in frequencies]
        ...
        drive_params = {
            ...
            'drive_description':
                "%d-point frequency sweep, %.4f to %.4f Hz at %.4E A rms"
                % (points, frequencies[0], frequencies[-1], current_rms),
        }
    ...
    def _record_point(self, point):
        ...
        self.data_storage['x'].append(point['frequency'])
        self.data_storage['y'].append(point['resistance'])
        self.data_storage['sub_x'].append(point['frequency'])
        self.data_storage['sub_y'].append(point[SUB_AXIS_KEY])
        self.line_main.set_data(
