"""
Module: Delta_RT_K6221_K2182_L340_Sensing_GUI.py
Purpose: Delta-mode R-T (Keithley 6221/2182) with passive temperature
         sensing from a Lake Shore Model 340.
         Port of Delta_RT_K6221_K2182_L350_Sensing_GUI.py to the Model 340
         command set.  The Keithley delta-mode logic is untouched; only the
...
-----------------------------------------------------
  ...
  * Heater-off at start is a checkbox, ON by default.  When ticked the
    ...
    exactly as found, which is what the 350 version always did.
  * The heater is never touched at stop or close.
...
class Combined_Backend:
    ...
    - Lakeshore 340 logic is passive temperature sensing only: *CLS, an
      optional verified RANGE 0 at start, then queries.  Never *RST.
    """
    ...
    def initialize_instruments(self, parameters):
        ...
        try:
            ...
            # --- Initialize Lakeshore 340 (Passive Mode) ---
            print("  Connecting to Lakeshore 340 for passive monitoring...")
            ...
            # command, printed 9-33); '\n' as read terminator works on the
            # lab's 340.
            ...
            if not self.is_model_340():
                raise RuntimeError(
                    ...
                    "Model 340. Refusing to send RANGE 0 to an unknown "
                    "instrument. Pick the right address.")
            # No *RST: on a 340 that resets loop, setpoint and ramp.
            ...
            if self.params.get('heater_off', True):
                ...
                if rng != 0:
                    raise RuntimeError(
                        f"Sent RANGE 0 but RANGE? reads back {rng}. Heater "
                        "is NOT off. Check the front panel (Remote/Local) "
                        "and try again.")
            ...
            print("  Lakeshore 340 connection is passive from here on: queries only.")
    ...
    def get_measurement(self):
        ...
        # Get data from Lakeshore 340: kelvin reading plus RDGST? status
        # (printed 9-41) so an out-of-range 0.000 K is never taken as real.
        sensor = self.params.get('sensor', 'A')
        temp_str = self.lakeshore.query(f'KRDG? {sensor}').strip()
        temperature = float(temp_str)
        status = int(float(self.lakeshore.query(f'RDGST? {sensor}').strip()))

        return resistance, voltage, temperature, status
    ...
    def close_instruments(self):
        ...
        if self.lakeshore:
            try:
                ...
                print("  Lakeshore 340 connection closed (was in passive mode).")
            except pyvisa.errors.VisaIOError:
                pass
            finally:
                self.lakeshore = None
...
class MeasurementAppGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("K6221/2182 & L340: Delta Mode R-T (Passive Sensing)")
    ...
    def create_header(self):
        ...
        Label(
            header_frame,
            text="K6221/2182 & L340: Delta Mode R-T (Passive Sensing)",
            bg=self.CLR_HEADER,
    ...
    def stop_measurement(self):
        if self.is_running:
            ...
            self.log("Instrument connections closed. "
                     "Lakeshore heater state was not touched at stop.")
            messagebox.showinfo(
    ...
    def _measurement_worker(self):
        ...
        while self.is_running:
            try:
                res, volt, temp, status = self.backend.get_measurement()
                elapsed = time.time() - self.start_time
                # Put the acquired data into the queue for the main thread
                self.data_queue.put((res, volt, temp, status, elapsed))
    ...
    def _handle_new_data_point(self, data):
        ...
        res, volt, temp, status, elapsed = data
    ...
    def _process_visa_queue(self):
        ...
        try:
            ...
            if isinstance(result, Exception):
                ...
                # Auto-select common addresses. Nothing is probed. The
                # lab's 340 is at address 19 (3 Sep 2026); 12 and 15 are
                # never auto-picked because other instruments live there.
                for res in result:
