...
# You can find the address by running:
...
def run_rescue(visa_address: str):
    ...
    try:
        ...
        # Set a short timeout so the script doesn't hang if the instrument is
        # truly frozen
        ...
        # It's often the most critical step to resolve a communication
        # deadlock.
        instrument.clear()
        ...
        while True:
            ...
            if '0,"No error"' in error_string:
                ...
                # If there was an error, print it. The query itself removes it
                # from the queue.
                print(
                    f"   - Found and cleared lingering error: {error_string.strip()}")
                error_count += 1

            # A safety break to prevent an infinite loop if the instrument is
            # misbehaving.
            if error_count > 10:
                print(
                    "   - More than 10 errors found. Stopping to prevent infinite loop.")
                break
        ...
        # This block ensures the connection is always closed, even if errors
        # occur.
        if instrument:
            instrument.close()
