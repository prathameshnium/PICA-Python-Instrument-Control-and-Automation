"""
Module: Sensor_Curve_Viewer_L340_GUI.py
Purpose: Look at the calibration curves that are already inside a Lake Shore
         Model 340 -- list every slot, read the breakpoints of one slot, plot
         them, and write them out as a .340 file and a CSV.
         Port of Sensor_Curve_Viewer_L350_GUI.py to the Model 340 command set.

What is different from the Model 350 version, and why
-----------------------------------------------------
  * Curve slots run 1 to 60 on a 340 (1-20 standard, 21-60 the forty user
    curves); the 350 stops at 59.  MAX_CURVE is 60 here and the slot list,
    the catalogue scan and every range check follow it.
                                  (340 manual, CRVHDR/CRVPT printed 9-30)
  * The connection is accepted only if *IDN? contains MODEL340 (case
    ignored, spaces ignored).  The 350 version accepted any Lake Shore; a
    viewer that reads a 350's or 336's slots as if they were a 340's would
    mis-number the last slot, so anything else is refused with its IDN.
  * INTYPE? <input> answers <type>,<units>,<coefficient>,<excitation>,<range>
    on a 340 (printed 9-34), with type codes 0 Special, 1 Si diode, 2 GaAlAs,
    3 Pt100 250 ohm, 4 Pt100 500 ohm, 5 Pt1000, 6 RhFe, 7 Carbon-Glass,
    8 Cernox, 9 RuOx, 10 Ge, 11 Capacitor, 12 Thermocouple.  The 350 reply
    (<type>,<autorange>,<range>,<compensation>,<units>, codes 0-5) is a
    different shape, so the channel panel decodes the 340 fields and names
    the sensor type.  Nothing is set: INTYPE? is a query.
  * There is no INNAME on a 340, so nothing asks for an input name.
  * CRVSAV is what commits a curve to flash on a 340.  This module never
    writes a curve, so it never sends CRVSAV either; it stays in the
    read-only guard's list of commands that are never a query.
  * The scan prefers the lab's 340 at IEEE address 19 (::19::, since
    3 Sep 2026) and never auto-picks an address on its own: only an *IDN?
    reply naming MODEL340 selects it.
  * Still no *RST, no *CLS.  On a 340 "*RST sets controller parameters to
    power-up settings" (printed 9-24), and a viewer opened during a run must
    not do that.  The read-only link refuses both before any I/O.

Commands used (all queries; Model 340 User's Manual, Chapter 9):
  *IDN?                 LSCI,MODEL340,<serial>,<firmware>
  CRVHDR? <curve>       name, serial, format, limit, coefficient   (9-30)
  CRVPT? <curve>,<n>    units value, temperature, n = 1-200        (9-30)
  INCRV? <input>        curve number on an input                  (9-33)
  INTYPE? <input>       type, units, coefficient, excitation, range (9-34)
         ...
         lot of knob turning:
           ...
           - which of the 60 curve slots hold anything, and what;
...
===============================================================================
WHAT THE MANUAL SAYS  (Lake Shore Model 340 User's Manual, Chapter 9)
===============================================================================

Curve slots
  1 to 60. Slots 1 to 20 hold the standard Lake Shore curves (DT-470, DT-500,
  PT-100, RX-102A, thermocouples and so on) and are read-only in the
  instrument itself. Slots 21 to 60 are the 40 user curves. This module makes
  ...
  how you find out what the instrument thinks a DT-470 is.
...
CRVHDR? <curve>                                              (printed 9-30)
  ...
  being invented on the way out.  (A curve WRITTEN to a 340 is only in
  flash after CRVSAV; a viewer never writes, so that never arises here.)

CRVPT? <curve>,<index>                                       (printed 9-30)
...
INCRV? <input>                                               (printed 9-33)
  Reply: the curve number that input A, B, C or D is using (C and D exist
  only with the 3462 option card). Read-only here; the module reports it
  so the curve on screen can be tied to a channel, and offers no way to
  change it.

INTYPE? <input>                                              (printed 9-34)
  Reply: <type>,<units>,<coefficient>,<excitation>,<range>. The type code
  is decoded with the table in SENSOR_TYPES_340; the other four fields are
  shown as the instrument gave them. Reported as context for the channel,
  again read-only.
...
Two files, both optional, both to a path chosen in a save dialog:
  ...
  .340   The Lake Shore breakpoint file. Same header keys and same three
         ...
         340 can go straight into another instrument. Written so that
  ...
  The curve list can also be written as a CSV of all 60 slots, which is the
  quickest way to keep a record of what a given instrument holds.

Author: Prathamesh Deshmukh
Version: 1.0  (Model 340 port, 4 Sep 2026)
"""
...
MIN_CURVE = 1
MAX_CURVE = 60               # Model 340: 1-20 standard, 21-60 user (9-30)
MAX_CURVE_OTHER = 59         # a 336 or 350 stops one slot earlier
FIRST_USER_CURVE = 21        # 1-20 are the standard Lake Shore curves
MAX_BREAKPOINTS = 200
INPUT_CHANNELS = ('A', 'B', 'C', 'D')   # C, D only with the 3462 option card

# INTYPE? <type> codes on a Model 340, manual printed 9-34.
SENSOR_TYPES_340 = {
    0: "Special",
    1: "Silicon Diode",
    2: "GaAlAs Diode",
...
INTYPE_FIELDS_340 = ('type', 'units', 'coefficient', 'excitation', 'range')


def max_curve_for_idn(idn):
    """The last curve slot on the instrument that answered '*IDN?'.

    A 340 has 60; a 336 or 350 stops at 59. connect() only ever accepts a
    340, so this is the guard that keeps the slot list honest should the
    identity check ever be loosened.
    """
    text = str(idn).upper().replace(' ', '').replace('-', '')
    return MAX_CURVE if 'MODEL340' in text else MAX_CURVE_OTHER


def describe_intype(reply):
    """Turn a 340 'INTYPE? <input>' reply into readable text.

    Reply shape (printed 9-34): <type>,<units>,<coefficient>,<excitation>,
    <range>. Only the type is decoded by name; the other four are numeric
    codes whose meaning depends on the type, so they are shown as given.
    Never raises: an odd reply comes back quoted, not invented.
    """
    raw = str(reply).strip()
    ...
    try:
        ...
        return f"unparseable INTYPE? reply '{raw}'"
    name = SENSOR_TYPES_340.get(code, f"unknown type {code}")
    text = f"type {code} = {name}"
    if len(parts) >= 5:
        rest = ", ".join(f"{field} {value}" for field, value
                         in zip(INTYPE_FIELDS_340[1:], parts[1:5]))
        text += f"; {rest}"
    elif len(parts) > 1:
        text += f"; other fields {','.join(parts[1:])}"
    return text
...
# Format 5 exists on the 340 (and not on a 350): its TEMPERATURE column is
# log10 of kelvin, so it is named here (a 340 can hold one) but never
# exported as a .340, because every reader of that file takes the column as
# kelvin.
CURVE_FORMATS = {
...
def build_340_text(header, points):
    ...
    out of the 340 here can be put into the Cryocon there with no editing.
    ...
    # coefficient or limit in a .340 would be loaded as if the 340 had said
...
def build_curve_csv(header, points, idn="", address=""):
    ...
    lines = [
        "# Lake Shore Model 340 calibration curve, read back over the bus",
        f"# Read on: {stamp}",
...
def build_catalogue_csv(entries, idn="", address=""):
    ...
    stamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    lines = [
        "# Lake Shore Model 340 curve list",
        f"# Read on: {stamp}",
        f"# Instrument: {idn or 'unknown'}",
        f"# VISA address: {address or 'unknown'}",
...
# '*IDN?' on a 340 answers 'LSCI,MODEL340,<serial>,<firmware>'. Only that
# is accepted: a 336 or 350 has 59 slots and a different INTYPE? shape.
...
# all default to 12. A hint only: the IDN check decides, and the scan never
# selects an address on the hint alone.
LAKESHORE_ADDRESS_HINT = "::19::"
...
def is_model_340_idn(idn):
    """True if a '*IDN?' reply names a Lake Shore Model 340 (case and
    spaces ignored)."""
...
class LakeshoreReadOnlyLink:
    """One paced VISA session to a Lake Shore 340 that can only ask questions.
...
class CurveViewerBackend:
    """Everything this module says to a Lake Shore 340. All of it queries."""
    ...
    def connect(self, visa_address):
        ...
        # something that is not a Model 340 returns whatever that
        # instrument makes of it, and reading a stranger's reply as a curve
        # header would put invented numbers on the screen. A 336 or 350
        # is refused too: it stops at slot 59 and its INTYPE? reply has a
        # different shape, so this viewer would mis-describe it.
        if not is_model_340_idn(idn):
            self.disconnect()
            raise ConnectionError(
                f"{visa_address} is not a Lake Shore Model 340: it identifies "
                f"itself as '{idn}'. Scan the bus and pick the 340's actual "
                f"address (the lab's 340 is at {LAKESHORE_ADDRESS_HINT}).")
    ...
    def read_channel_curves(self):
        ...
        configuration (<type>,<units>,<coefficient>,<excitation>,<range> on
        a 340, printed 9-34; decoded by describe_intype). Neither is changed
        here; they are reported so a curve on screen can be tied to a
        channel. Inputs C and D only exist with the 3462 option card; on a
        base 340 they simply do not answer and are shown as such.
        """
        if not self.link:
            raise ConnectionError("Not connected to instrument.")
        result = {}
        for channel in INPUT_CHANNELS:
            ...
            if str(entry['intype']).startswith('<no answer'):
                entry['intype_text'] = entry['intype']
            else:
                entry['intype_text'] = describe_intype(entry['intype'])
...
class CurveViewerGUI:
    """Browse and export the curves already inside a Lake Shore 340.
    ...
    PROGRAM_VERSION = "1.0"
    PROGRAM_NAME = "Lakeshore 340 Sensor Curve Viewer"
    ...
    def _create_info_panel(self, parent, grid_row):
        ...
        ttk.Label(frame,
                  text=(f"Lake Shore Model 340 | curve slots {MIN_CURVE}-"
                        ...
                        "are the user curves.\n"
                        "Inputs A, B (C, D only with the 3462 option card)."),
    ...
    def _require_connection(self):
        if not self.is_connected or not self.backend.is_connected:
            ...
            messagebox.showerror("Not Connected",
                                 "Connect to the Lake Shore 340 first "
                                 "(step 1).")
            return False
        return True
    ...
    def _scan_visa(self):
        ...
        # Only an *IDN? reply naming MODEL340 selects an address; the lab's
        # ::19:: is preferred if more than one answers. Nothing is picked on
        # the address alone (12 and 15 are shared by other instruments).
        candidates = [r for r in resources
                      if is_model_340_idn(identities.get(r, ''))]
        lakeshore = next((r for r in candidates
                          if LAKESHORE_ADDRESS_HINT in r), None)
        if lakeshore is None and candidates:
            lakeshore = candidates[0]
        if lakeshore:
            self.visa_cb.set(lakeshore)
            self.log(f"Lake Shore Model 340 identified at {lakeshore} and "
                     "selected.")
            return
        self.visa_cb.set('')
        self.log("WARNING: no address answered *IDN? as a Lake Shore "
                 f"MODEL340 (the lab's 340 is at {LAKESHORE_ADDRESS_HINT}). "
                 "Pick it yourself; Connect will refuse anything else.")
    ...
    def _do_connect(self):
        ...
        try:
            ...
            # The slot list follows the instrument: 60 on a 340 (only a 340
            # gets this far; max_curve_for_idn would say 59 for a 336/350).
    ...
    def _default_stem(self):
        ...
        return f"L340_slot{header.get('curve', 0):02d}_{name}"
    ...
    def _export_catalogue_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"L340_curve_list_{stamp}.csv",
            filetypes=[("CSV", "*.csv"), ("All files", "*.*")])
        if not path:
            return
...
#
#     python Sensor_Curve_Viewer_L340_GUI.py --selftest
#
...
def _selftest_cases():
    ...
    def case_link_refuses():
        class FakeInstrument:
            ...
            def query(self, command):
                ...
                return "LSCI,MODEL340,340219,111196"
    ...
    def case_340_slots_and_incomplete_reads():
        check(max_curve_for_idn("LSCI,MODEL336,LSA23AR,1.5") == 59, "336")
        check(max_curve_for_idn("LSCI,MODEL340,340219,111196") == 60, "340")
        check(max_curve_for_idn("LSCI,MODEL 340,1,1") == 60, "spaced 340")
        check(max_curve_for_idn("") == MAX_CURVE_OTHER, "unknown falls back")
        check(is_model_340_idn("LSCI,MODEL340,340219,111196"), "340 idn")
        check(is_model_340_idn("lsci,model 340,1,1"), "spaced, lower case")
        check(not is_model_340_idn("LSCI,MODEL336,LSA23AR,1.5"), "336 refused")
        check(not is_model_340_idn("Cryocon Model 34, Rev 3.03A"), "CC34 refused")
        ...
        try:
            ...
            raise AssertionError("slot 60 must be refused when the "
                                 "instrument stops at 59")
    ...
    def case_curve_csv():
        ...
        text = build_curve_csv(header, [(1.0, 300.0), (2.0, 100.0)],
                               idn="LSCI,MODEL340", address="GPIB0::19::INSTR")
    ...
    def case_catalogue_csv():
        ...
        text = build_catalogue_csv(entries, idn="LSCI",
    ...
    # -- 17: the 340 INTYPE? reply is decoded, never invented --------------
    def case_intype_340():
        text = describe_intype("8,2,1,6,8")
        check(text.startswith("type 8 = Cernox"), text)
        check("units 2" in text and "range 8" in text, text)
        check(describe_intype("1,1,1,0,0").startswith("type 1 = Silicon Diode"),
              describe_intype("1,1,1,0,0"))
        check("unknown type 99" in describe_intype("99,1,1,1,1"),
              describe_intype("99,1,1,1,1"))
        check("unparseable" in describe_intype("junk"), describe_intype("junk"))
