...
class TempControlGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(
            f"Lakeshore 350 Temperature Control v{self.PROGRAM_VERSION}")
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            header,
            text="Lakeshore 350 Temperature Ramp Utility",
            style='Header.TLabel',
    ...
    def _create_control_panel(self, parent, grid_row):
        ...
        heater_cb = ttk.Combobox(frame, textvariable=self.heater_range_var, values=[
                                 'Off', 'Low', 'Medium', 'High'], state='readonly')
    ...
    def start_ramp(self):
        try:
            ...
            self.backend.configure_ramp(
                self.params['setpoint'],
                self.params['rate'],
                self.params['heater_range'])
            self.log(
                f"Ramp started towards {self.params['setpoint']} K at {self.params['rate']} K/min.")
    ...
    def stop_ramp(self):
        ...
        messagebox.showinfo(
            "Ramp Stopped",
            "The temperature ramp has been stopped and the heater is off.")
    ...
    def _monitoring_loop(self):
        ...
        try:
            ...
            if (self.params['rate'] > 0 and temp >= self.params['setpoint']) or (
                ...
                messagebox.showinfo(
                    "Ramp Complete",
                    f"Target temperature of {self.params['setpoint']} K has been reached.")
            else:
                self.root.after(
    ...
    def _on_closing(self):
        if self.is_running and messagebox.askyesno(
                "Exit", "A ramp is active. Stop and exit?"):
            self.stop_ramp()
            self.root.destroy()
