"""
Module: T_Control_L340_Step_GUI_advanced.py
Purpose: ADVANCED Lakeshore 340 temperature step-sequence controller.
         Port of T_Control_L350_Step_GUI_advanced.py to the Model 340
         command set.  Fully self-contained (no imports from other PICA
         programs).
...
-----------------------------------------------------
  ...
  * There is no TLIMIT on a 340.  "CLIMIT? 1" is read at start and its
    setpoint limit, max current code and max range are logged; a step (or
    a 'from above' pre-target) above the setpoint limit, or a heater range
    above the max range, is refused before the first ramp.         (9-28)
  * A 340 ramps from the CURRENT SETPOINT, not from the temperature.  Each
    ramp stage therefore pins the setpoint to the present reading with the
    ramp off (RAMP 1,0,0; SETP 1,<T now>) before enabling the ramp and
    sending the (pre-)target, exactly as T_Control_L340_RangeControl_GUI.py
    does.  The stage-2 handover (setpoint already at the pre-target, i.e.
    within tolerance of the reading) only changes the rate and target.
  * PID limits are 0-1000 for P, I and D (350: P<=9999, D<=200); the ramp
    rate is 0.1-100 K/min (350: 0.001-100).  Any computed rate below 0.1
    K/min is raised to 0.1 with a log line rather than aborting a run.
                                                                   (9-40)
  * HTRST? is read on every poll; a change is logged once, a non-zero code
    ...
    sample is not used for the stability window or pre-target test.
                                                                   (9-41)
  ...
  * No *RST anywhere: on a 340 "*RST sets controller parameters to
    ...
    Only *CLS is sent.
  * The connection is accepted only if *IDN? contains MODEL340 (the 350
    file merely printed a warning).  Scan pre-selects the lab address
    (::19::) and never auto-picks 12 or 15; Identify confirms by *IDN?.
  * MODE is not sent (the 350 file did not send it either).       (9-38)
  * The hardcoded 340 K kill switch is a temperature, unrelated to the
    model number; it is unchanged.

This program supersedes T_Control_L340_Step_GUI.py for demanding runs
...
class Lakeshore_Backend:
    ...
    HARD_TEMP_LIMIT_K = 340.0  # hardcoded kill switch (kelvin) — not in the GUI
    RAMP_MIN = 0.1             # RAMP rate limits on a Model 340, K/min (9-40)
    RAMP_MAX = 100.0
    ...
    @classmethod
    def clamp_rate(cls, rate):
        """Clamp a rate to the 340's RAMP range.  Returns (rate, msg|None).
        A computed rate below 0.1 K/min is raised, not refused, so an
        overnight run is never aborted by a first-step factor."""
        if rate < cls.RAMP_MIN:
            return cls.RAMP_MIN, (f"rate {rate:g} K/min below the Model 340 "
                                  f"minimum; using {cls.RAMP_MIN:g} K/min")
        if rate > cls.RAMP_MAX:
            return cls.RAMP_MAX, (f"rate {rate:g} K/min above the Model 340 "
                                  f"maximum; using {cls.RAMP_MAX:g} K/min")
        return rate, None
    ...
    def configure_ramp(self, setpoint, rate, heater_range, current_temperature):
        ...
        Returns the rate actually sent (clamped to 0.1-100 K/min)."""
        self.set_heater_range(heater_range)
        rate, _ = self.clamp_rate(rate)
    ...
    def get_status(self, control_input, retries=2):
    ...
    def set_pid(self, loop, p, i, d):
        ...
        # NOTE: Lake Shore controllers treat I=0 / D=0 as "off". Do not set
        # I=0 unless you intend to disable integral action (permanent offset).
...
class TempControlAdvancedGUI:
    ...
    # The Lake Shore integral term scales with BOTH P and I, and the LN2 probe
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(
            f"Lakeshore 340 ADVANCED Step Control v{self.PROGRAM_VERSION}"
        )
        ...
        self._last_htr_error = 0    # HTRST? code, logged once on change
    ...
    def create_widgets(self):
        ...
        ttk.Label(header,
                  text="Lakeshore 340 Advanced Step Sequence Utility",
                  style="Header.TLabel",
    ...
    def _create_info_panel(self, parent, grid_row):
        ...
        ttk.Label(frame,
                  text="Lake Shore Model 340 | Loop 1 heater | "
                       "CSET/CMODE/CLIMIT at start",
                  font=("Segoe UI", 9), background=self.CLR_FRAME_BG
    ...
    def _create_safety_panel(self, parent, grid_row):
        ...
        ttk.Label(frame, text="Control Input:").grid(row=3, column=0, sticky="w",
                                                     padx=10, pady=5)
        self.input_var = tk.StringVar(value="A")
        self.input_cb = ttk.Combobox(frame, textvariable=self.input_var,
                                     values=["A", "B", "C", "D"],
                                     state="readonly", width=5)
        self.input_cb.grid(row=3, column=1, sticky="w", padx=5)
        self.identify_btn = ttk.Button(frame, text="Identify",
                                       command=self._identify_visa)
        self.identify_btn.grid(row=3, column=4, columnspan=2, sticky="ew", padx=5)

        ttk.Button(frame, text="Browse Save…", command=self._browse_save).grid(
            row=4, column=0, columnspan=2, sticky="ew", padx=10, pady=(2, 2))
        self.save_dir_lbl = ttk.Label(frame, text="Save dir: (not set)",
                                      foreground=self.CLR_ACCENT_GOLD,
                                      font=("Segoe UI", 8))
        self.save_dir_lbl.grid(row=4, column=2, columnspan=4, sticky="w", padx=5)
    ...
    def _validate_and_get_params(self):
        ...
        if not (Lakeshore_Backend.RAMP_MIN <= p["app_rate"]
                <= Lakeshore_Backend.RAMP_MAX):
            raise ValueError("Approach rate must be 0.1-100 K/min on a Model 340.")
        if p["stab_timeout"] < 0:
        ...
        if (p["min_rate"] < Lakeshore_Backend.RAMP_MIN
                or p["max_rate"] < p["min_rate"]
                or p["max_rate"] > Lakeshore_Backend.RAMP_MAX):
            raise ValueError("Need 0.1 <= Min Rate <= Max Rate <= 100 K/min "
                             "on a Model 340.")
        if not (Lakeshore_Backend.RAMP_MIN <= p["rate"]
                <= Lakeshore_Backend.RAMP_MAX):
            raise ValueError("Fixed rate must be 0.1-100 K/min on a Model 340.")
        if p["heater_range"] == "0":
        ...
        try:
            while True:
                ...
                if kind == "stop":
                    ...
                    try:
                        new_code = self.backend.range_code(cmd[1])
                        if self.climit and new_code > self.climit["max_range"]:
                            raise ValueError(
                                f"range {new_code} exceeds CLIMIT max range "
                                f"{self.climit['max_range']}")
                        self.backend.set_heater_range(new_code)
                        self.params["heater_range"] = str(new_code)
                        self._put_gui_msg("log",
                                          text=f"Heater range set: RANGE "
                                               f"{new_code} (verified)")
                    except Exception as e:
                        self._put_gui_msg("log",
                    ...
                    if "rate" in updates and not self.params["adaptive"]:
                        if self._phase == "FINAL_APPROACH":
                            ...
                            try:
                                ...
                                new_rate, lim_msg = \
                                    self.backend.clamp_rate(new_rate)
                                if lim_msg:
                                    self._put_gui_msg("log", text=f"Live {lim_msg}")
    ...
    def _read_temp_point(self, target):
        ...
        Returns the temperature, or None when RDGST? flags the reading
        (invalid/old/over-range): such a sample is logged and plotted but
        must not drive the pre-target test or the stability window."""
        temp, resistance, htr, rd_status = self.backend.get_status(
            self.params["input"])

        # Heater error register (HTRST?): logged once on change, no dialog.
        ...
        if rd_status:
            self._put_gui_msg(
                "log",
                text=f"T:{temp:.3f} K   [RDGST {rd_status}] - sample not "
                     "used for stability")
            return None
        return temp

    def _read_valid_temperature(self, target, attempts=5):
        """A reading RDGST? calls good, retrying briefly; raises otherwise."""
        ...
        raise RuntimeError(
            f"Input {self.params['input']} gave no valid reading in "
            f"{attempts} polls (see RDGST lines in the console).")
    ...
    def _ramp_and_stabilize(self, target, ramp_rate):
        ...
        temp = self._read_valid_temperature(target)
        ...
        # CLIMIT setpoint limit applies to the pre-target as well
        # ('from above' places it above the setpoint).
        if pre_target is not None and pre_target > self.climit["sp_limit"]:
            raise ValueError(
                f"Pre-target {pre_target:.2f} K is above the 340's setpoint "
                f"limit of {self.climit['sp_limit']:g} K (CLIMIT).")

        # Each stage pins the setpoint to the present temperature before
        # enabling the ramp (a 340 ramps from the current setpoint).
        if pre_target is not None:
            self._phase = "PRE_RAMP"
            ramp_rate, lim_msg = self.backend.clamp_rate(ramp_rate)
            if lim_msg:
                self._put_gui_msg("log", text=f"Stage 1 {lim_msg}")
            self._current_rate = ramp_rate
            self.backend.configure_ramp(pre_target, ramp_rate,
                                        p["heater_range"], temp)
            self._put_gui_msg(
                "log",
                text=f"Stage 1: setpoint pinned to {temp:.3f} K, ramp "
                     f"({ramp_rate:g} K/min) to pre-target "
            ...
            app_rate, lim_msg = self.backend.clamp_rate(app_rate)
            if lim_msg:
                self._put_gui_msg("log", text=f"Approach {lim_msg}")
            self._current_rate = app_rate
            self.backend.configure_ramp(target, app_rate,
                                        p["heater_range"], temp)
            self._put_gui_msg(
                "log",
                text=f"Already within approach band — setpoint pinned to "
                     f"{temp:.3f} K, slow approach ({app_rate:g} K/min) "
                     f"to {target} K.")
        ...
        try:
            while self.is_running:
                ...
                if temp is None:
                    # RDGST? flagged the sample: neither pre-target test
                    # nor stability window may use it.  Trim the window
                    # by time only and poll again.
                ...
                if self._phase == "PRE_RAMP":
                    ...
                    if reached:
                        ...
                        self._phase = "FINAL_APPROACH"
                        self._current_rate = app_rate
                        # Setpoint is already at the pre-target (within
                        # tolerance of the reading): no pin needed here.
        ...
        try:
            ...
            self._put_gui_msg(
                ...
                text=f"Loop 1 enabled on input {cset['input']} (kelvin), "
                     f"Manual PID (CMODE {cmode}). CLIMIT: setpoint <= "
                     f"{limits['sp_limit']:g} K, max current {cur}, "
                     f"max range {limits['max_range']}.")
            if self.params["max_temp"] > limits["sp_limit"]:
                self._put_gui_msg(
                    "log",
                    text=f"NOTE: Max Temp {self.params['max_temp']:g} K is "
                         f"above the 340's CLIMIT setpoint limit "
                         f"{limits['sp_limit']:g} K; the instrument limit "
                         "wins for setpoints.")
            with self.setpoint_lock:
                planned = list(self.setpoint_floats)
            band = self.params["app_band"] \
                if self.params["approach_side"] == "above" else 0.0
            too_hot = [t for t in planned if t + band > limits["sp_limit"]]
            if too_hot:
                raise ValueError(
                    f"Setpoint(s) {too_hot} K"
                    f"{' (+ from-above band)' if band else ''} are above the "
                    f"340's setpoint limit of {limits['sp_limit']:g} K "
                    "(CLIMIT). Remove them or raise the limit in the Direct "
            ...
            self._put_gui_msg("status",
                              text="ERROR: SEQUENCE STOPPED, HEATER OFF "
                                   "(see console)",
                              color=self.CLR_ACCENT_RED)
            self._put_gui_msg("beep")
        finally:
