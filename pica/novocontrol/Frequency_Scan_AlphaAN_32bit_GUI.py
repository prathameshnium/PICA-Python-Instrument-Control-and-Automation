'''
 PROGRAM:      Novocontrol Alpha-AN Frequency Scan GUI (32-bit, raw NI-488.2)
 EDITION:      This is the 32-bit / no-VISA twin of
               Frequency_Scan_AlphaAN_GUI.py, for the Novocontrol BDS PC
               whose only working GPIB stack is a 32-bit gpib-32.dll with no
               VISA GPIB interface configured. There
               pyvisa.ResourceManager().list_resources() returns nothing
               however healthy the bus is. Everything below the transport is
               identical to the VISA edition: same command flow, same safety
               ceilings, same ack protocol, same WinDETA export.

               RUN IT IN 32-BIT PYTHON:

                   py -3.10-32 Frequency_Scan_AlphaAN_32bit_GUI.py

               matplotlib (and Pillow, for the logo) must be installed in
               that interpreter. A 64-bit run reports the wrong bitness and
               refuses to open rather than failing obscurely.

               The two editions are separate files by choice, so the proven
               VISA program is never disturbed by work on this one. A fix to
               the sweep logic, the safety ceilings or the export format has
               to be made in BOTH.
...
def launch_gpib_scanner():
    ...
    try:
        ...
        scanner_path = os.path.join(
            script_dir, "..", "utils", "GPIB_Scanner_32bit_GUI.py"
        )
...
# ===============================================================================
# BACK END TRANSPORT - raw NI-488.2 through ctypes. No pyvisa, no VISA.
#
# On the Novocontrol BDS PC the only working GPIB stack is a 32-bit
# gpib-32.dll with no VISA GPIB interface configured, so
# pyvisa.ResourceManager().list_resources() returns nothing however healthy
# the bus is. AlphaGpibSession below presents the same handful of methods the
# measurement code already uses (write / read / query / read_stb /
# wait_for_srq / close, plus the timeout and termination attributes), so every
# command site in this file is byte-for-byte the same as in the VISA edition.
#
# A 32-bit DLL can only be driven from 32-bit Python. Run this module with
# 'py -3.10-32'; matplotlib (and Pillow, for the logo) must be installed in
# that interpreter.
# ===============================================================================
...
RQS = 1 << 11    # device is requesting service (SRQ)
CMPL = 1 << 8    # I/O complete

# --- NI-488.2 timeout codes, seconds -> code ---
# The Alpha needs a long ceiling: a single MTM point at the bottom of the
# sweep is slow, and the VISA edition sets 60 s. There is no 60 s code, so
# _timeout_code always rounds UP (60 s -> T100s). Rounding down would abort
# legitimate low-frequency points.
TIMEOUT_CODES = ((0.3, 10), (1, 11), (3, 12), (10, 13), (30, 14),
                 (100, 15), (300, 16), (1000, 17))
...
# Boards looked at by the bus census, in order.
SCAN_BOARDS = (0, 1)


class GpibError(RuntimeError):
    """Any failure reported by the NI-488.2 driver."""
...
def sta_text(sta):
    bits = [name for mask, name in ((ERR, "ERR"), (TIMO, "TIMO"),
                                    (END, "END"), (RQS, "RQS"),
                                    (CMPL, "CMPL")) if sta & mask]
    return f"0x{sta & 0xFFFF:04X} [{' '.join(bits) or 'none'}]"


def _timeout_code(seconds):
    """Nearest NI-488.2 timeout code at or ABOVE the requested seconds."""
    for limit, code in TIMEOUT_CODES:
        if seconds <= limit:
            return code
    return TIMEOUT_CODES[-1][1]


def find_dll_candidates():
    """The standard GPIB DLL locations, tried once each -- no disk hunting.

    From a 32-bit process Windows redirects System32 to SysWOW64, so the
    System32 entry is what actually delivers the 32-bit gpib-32.dll.
    SysWOW64 is listed too, so a 64-bit run can at least report that the
    32-bit library exists (it refuses to load it, WinError 193).
    """
...
class RawGpib:
    """Minimal ctypes binding of the traditional NI-488.2 API.

    Deliberately does not touch ibfind: the 32-bit library on the Novocontrol
    PC exports ibfindA/ibfindW only, so board-level work goes through the
    ...
    def __init__(self, dll_path):
        ...
        self.ibwait = _bind(lib, "ibwait", [c_int, c_int])
        self.ibrsp = _bind(lib, "ibrsp", [c_int, ctypes.POINTER(ctypes.c_char)])
        self.ibln = _bind(
            lib, "ibln",
            [c_int, c_int, c_int, ctypes.POINTER(ctypes.c_short)])

        missing = [name for name in ("ibdev", "ibonl", "ibwrt", "ibrd",
                                     "ibtmo", "ibwait", "ibrsp", "ibln")
    ...
    def listener_at(self, board, pad):
        ...
        Returns (present, ibsta); 'present' is None when the call errored,
        which means the board is unusable, not that the address is empty.
        """
...
def load_gpib_driver():
    """Returns (RawGpib or None, list of report lines). Never raises."""
    bits = struct.calcsize("P") * 8
    lines = []
    if os.name != "nt":
        return None, ["This edition drives the Windows NI-488.2 DLL; "
                      "on Linux use linux-gpib."]
    candidates = find_dll_candidates()
    if not candidates:
        return None, ["No GPIB driver DLL found in System32 or SysWOW64."]
    for path in candidates:
        try:
            ...
            if getattr(exc, "winerror", None) == 193:
                lines.append(f"{path}: WRONG BITNESS for this {bits}-bit "
                             "Python. Run this module with 'py -3.10-32'.")
            else:
        ...
        try:
            ...
            continue
        lines.append(f"GPIB driver in use: {path}")
        return driver, lines
    return None, lines


def parse_gpib_address(address):
    """'GPIB0::5::INSTR' or 'GPIB::5' or '5' -> (board, primary address)."""
    text = str(address).strip().upper()
    parts = [p for p in text.replace("::", ":").split(":") if p]
    board, pad = 0, None
    for part in parts:
        if part.startswith("GPIB"):
            tail = part[4:]
            if tail.isdigit():
                board = int(tail)
        elif part.isdigit():
            ...
            continue
    if pad is None:
        raise ValueError(f"Cannot read a GPIB address out of {address!r}. "
                         f"Expected something like GPIB0::5::INSTR.")
    if not 1 <= pad <= 30:
        raise ValueError(f"GPIB primary address {pad} is out of range 1-30.")
    return board, pad


class AlphaGpibSession:
    """A pyvisa-shaped session over the raw NI-488.2 driver.

    Only the surface this module actually uses is implemented. The framing is
    fixed at open time to what the Alpha-AN requires -- EOI on writes
    (eot=1), no EOS byte (eos=0) -- so read_termination / write_termination /
    send_end are accepted for source compatibility and, being already true of
    the transport, change nothing.
    """

    # Padding the Alpha adds around EOI-terminated replies.
    RESPONSE_STRIP = b" \t\r\n\x00\x10"

    def __init__(self, driver, board, pad, timeout_ms=60000):
        self._driver = driver
        self.board = board
        self.pad = pad
        self.resource_name = f"GPIB{board}::{pad}::INSTR"
        self._timeout_ms = timeout_ms
        self._handle = driver.open_device(
            board, pad, _timeout_code(timeout_ms / 1000.0))
        if self._handle < 0:
            raise GpibError(
                f"Could not open {self.resource_name}: "
                f"{iberr_text(driver.error_code())}")
        # Present for source compatibility with the VISA edition.
        self.read_termination = ""
        self.write_termination = ""
        self.send_end = True

    # --- pyvisa-shaped attributes ---
    @property
    def timeout(self):
    ...
    def timeout(self, milliseconds):
        ...
        self._set_timeout_seconds(milliseconds / 1000.0)

    def _set_timeout_seconds(self, seconds):
        sta = self._driver.ibtmo(self._handle, _timeout_code(seconds))
        if sta & ERR:
            raise GpibError(f"ibtmo failed: {sta_text(sta)}; "
                            f"{iberr_text(self._driver.error_code())}")

    # --- transfers ---
    def write(self, command):
        payload = command.encode("ascii")
        sta = self._driver.ibwrt(self._handle, payload, len(payload))
        if sta & ERR:
            raise GpibError(
                f"write {command!r} failed: {sta_text(sta)}; "
                f"{iberr_text(self._driver.error_code())}")
        return len(payload)

    def read(self, max_len=65536):
        buffer = ctypes.create_string_buffer(max_len)
        sta = self._driver.ibrd(self._handle, buffer, max_len)
        if sta & ERR:
            raise GpibError(
                f"read failed: {sta_text(sta)}; "
                f"{iberr_text(self._driver.error_code())}")
        count = self._driver.count()
        if count is not None and 0 <= count <= max_len:
            data = buffer.raw[:count]
        else:
            data = buffer.raw.rstrip(b"\x00")
        return data.strip(self.RESPONSE_STRIP).decode("ascii", "replace")
    ...
    def read_stb(self):
        """Serial poll. Also clears the device's pending service request."""
        status = ctypes.c_char()
        sta = self._driver.ibrsp(self._handle, ctypes.byref(status))
        if sta & ERR:
            raise GpibError(
                f"serial poll failed: {sta_text(sta)}; "
                f"{iberr_text(self._driver.error_code())}")
        return ord(status.value) if status.value else 0

    def wait_for_srq(self, timeout_ms=25000):
        """Block until this device requests service, or raise TimeoutError.

        ibwait takes its deadline from the device timeout, so the requested
        window is installed first and the working timeout restored after.
        """
        previous = self._timeout_ms
        self._set_timeout_seconds(timeout_ms / 1000.0)
        try:
            sta = self._driver.ibwait(self._handle, RQS | TIMO)
            if sta & ERR and not sta & TIMO:
                raise GpibError(
                    f"ibwait failed: {sta_text(sta)}; "
                    f"{iberr_text(self._driver.error_code())}")
            if not sta & RQS:
                raise TimeoutError(
                    f"No SRQ within {timeout_ms / 1000.0:.0f} s "
                    f"({sta_text(sta)}).")
        finally:
            try:
                self._set_timeout_seconds(previous / 1000.0)
            except GpibError:
                pass
    ...
    def close(self):
        if self._handle is not None and self._handle >= 0:
            try:
                self._driver.ibonl(self._handle, 0)
            finally:
                self._handle = None


# --- Packages for Back end ---
# The driver is loaded once, at import, exactly as the VISA edition creates
# its ResourceManager once. Loading a DLL touches no instrument.
GPIB_DRIVER, GPIB_DRIVER_REPORT = load_gpib_driver()
GPIB_AVAILABLE = GPIB_DRIVER is not None
...
class AlphaAN_Backend:
    ...
    def __init__(self):
        ...
        self.params = {}
        # The loaded NI-488.2 driver, standing in for the VISA edition's
        # ResourceManager. None when no usable DLL was found.
        self.driver = GPIB_DRIVER
        ...
        for line in GPIB_DRIVER_REPORT:
            print(f"  {line}")
    ...
    def connect(self, visa_addr):
        ...
        if not self.driver:
            raise ConnectionError(
                "No usable NI-488.2 driver. "
                + " ".join(GPIB_DRIVER_REPORT))

        board, pad = parse_gpib_address(visa_addr)
        inst = AlphaGpibSession(self.driver, board, pad)
        # 60 s; a long MTM at low frequency is slow. NI-488.2 timeouts are
        # discrete and there is no 60 s code, so this installs T100s: the
        # ceiling always rounds up, never down onto T30s.
        inst.timeout = 60000
        ...
        # ibdev already opened the device that way (eot=1, eos=0); these three
        # are set for parity with the VISA edition and change nothing.
...
class AlphaAN_FreqScan_GUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Novocontrol Alpha-AN Frequency Scan (32-bit GPIB)")
    ...
    def _scan_for_visa(self):
        """Bus census: which addresses are occupied. Sends nothing.

        The VISA edition queries *IDN? on every resource it enumerates and
        auto-selects whatever answers 'NOVOCONTROL'. That is not possible
        here and would not be wanted anyway: this edition uses ibln, which
        asserts a listen address and watches the NDAC handshake line without
        putting a single byte on the bus. The Alpha answers its own command
        set rather than SCPI, so a listener is the evidence; identity is
        established by the *IDN? and INTTYP? that Connect already sends.
        """
        if not GPIB_AVAILABLE or self.backend.driver is None:
            self.log("ERROR: no usable NI-488.2 driver.")
            for line in GPIB_DRIVER_REPORT:
                self.log(f"  {line}")
            return

        self.log("Scanning the GPIB bus (listen handshake only; nothing is "
                 "sent to any instrument)...")
        driver = self.backend.driver
        found = []

        try:
            for board in SCAN_BOARDS:
                probe, sta = driver.listener_at(board, 1)
                if probe is None:
                    self.log(f"  board {board} unusable: {sta_text(sta)}; "
                             f"{iberr_text(driver.error_code())}")
                    continue
                for pad in SCAN_ADDRESSES:
                    present, _sta = driver.listener_at(board, pad)
                    if present:
                        label = f"GPIB{board}::{pad}::INSTR"
                        found.append(label)
                        self.log(f"  listener at {label}")

            self.visa_combobox["values"] = found

            if len(found) == 1:
                self.visa_combobox.set(found[0])
                self.log("One instrument on the bus; selected. Press Connect "
                         "to confirm it is an Alpha with a ZG4.")
            elif found:
                self.log(
                    f"{len(found)} instruments on the bus. Choose the "
                    "Alpha's address (it is in WinDETA's device settings, "
                    "historically 5), then press Connect."
                )
            else:
                self.log("No listeners. Check instrument power, cable "
                         "seating, the address set on the instrument, and "
                         "that no other GPIB program (WinDETA) holds the "
                         "bus.")
...
def main():
    ...
    if not GPIB_AVAILABLE:
        root = tk.Tk()
        root.withdraw()
        detail = "\n".join(GPIB_DRIVER_REPORT) or "No GPIB DLL found."
        if bits == 64:
            advice = (
                "\n\nThis is 64-bit Python. The lab's driver is the 32-bit "
                "gpib-32.dll, which can only be used from 32-bit Python:"
                "\n\n    py -3.10-32 Frequency_Scan_AlphaAN_32bit_GUI.py"
                "\n\nmatplotlib (and Pillow) must be installed in that "
                "interpreter."
            )
        else:
            advice = ("\n\nInstall or repair the GPIB driver, then reopen "
                      "this window.")
        messagebox.showerror("No GPIB driver", detail + advice)
        return
