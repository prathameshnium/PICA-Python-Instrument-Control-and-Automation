...
from tkinter import ttk, Label, Entry, Button, filedialog, messagebox, scrolledtext
import numpy as np
import os
import time
import traceback
...
class ExperimentBackend:
    ...
    def set_temperature(self, target_temp_k):
        if self.lakeshore:
            self.lakeshore.write(f"SETP 1,{target_temp_k}"); self.lakeshore.write("RANGE 1,5")
    ...
    def run_iv_sweep(self, iv_params):
        ...
        self.keithley.source_current_range = max_a if max_a > 1e-6 else 1e-6
        self.keithley.compliance_voltage = iv_params['compliance_v']
        self.keithley.source_current = 0; self.keithley.enable_source(); self.keithley.measure_voltage()
        time.sleep(1)

        fwd = np.arange(0, max_a + step_a, step_a)
        rev = np.arange(max_a - step_a, 0 - step_a, -step_a)
        sweep = np.concatenate([fwd, rev])

        for i_set in sweep:
            self.keithley.ramp_to_current(i_set, steps=5, pause=0.05)
            time.sleep(iv_params['delay_s'])
            voltage_data.append(self.keithley.voltage)
...
class TemperatureIVGUI:
    ...
    def setup_styles(self):
        ...
        style.configure('TLabel', background=self.CLR_BG_DARK, foreground=self.CLR_FG_LIGHT)
    ...
    def create_parameter_frames(self, parent):
        ...
        self.start_button = ttk.Button(control_frame, text="Start", style='Start.TButton', command=self.start_experiment)
        self.start_button.grid(row=0, column=1, padx=5, sticky='ew')
        self.stop_button = ttk.Button(control_frame, text="Stop", style='Stop.TButton', state='disabled', command=self.stop_experiment)
    ...
    def create_graph_frame(self, parent):
        ...
        self.ax_main.set_title("Waiting for experiment...", fontweight='bold', color=self.CLR_BG_DARK)
        self.ax_main.set_xlabel("Current (A)", color=self.CLR_BG_DARK); self.ax_main.set_ylabel("Voltage (V)", color=self.CLR_BG_DARK)
        self.ax_main.tick_params(colors=self.CLR_BG_DARK)
        self.figure.tight_layout()
        self.canvas = FigureCanvasTkAgg(self.figure, container); self.canvas.get_tk_widget().pack(fill='both', expand=True)
